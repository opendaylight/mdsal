...
public interface BindingRuntimeTypes extends EffectiveModelContextProvider, RuntimeTypeContainer, Immutable {
    ...
    default Optional<AugmentRuntimeType> findAugmentation(final JavaTypeName typeName) {
        return findSchema(typeName).filter(AugmentRuntimeType.class::isInstance).map(AugmentRuntimeType.class::cast);
    }
