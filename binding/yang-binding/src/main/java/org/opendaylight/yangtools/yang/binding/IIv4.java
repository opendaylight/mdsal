...
sealed class IIv4<T extends DataObject> implements Externalizable permits KIIv4 {
    ...
    private @Nullable Iterable<ContainerStep<?>> pathArguments;
    ...
    final Iterable<ContainerStep<?>> getPathArguments() {
    ...
    public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        final var builder = ImmutableList.<ContainerStep<?>>builderWithExpectedSize(size);
        for (int i = 0; i < size; ++i) {
            builder.add((ContainerStep<?>) in.readObject());
        }
