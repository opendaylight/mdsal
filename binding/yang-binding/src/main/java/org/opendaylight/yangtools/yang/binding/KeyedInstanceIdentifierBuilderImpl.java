...
public class KeyedInstanceIdentifierBuilderImpl<T extends DataObject & Identifiable<K>,
    ...
    public @NonNull KeyedInstanceIdentifier<T, K> build() {
        if (arg instanceof InstanceIdentifier.IdentifiableItem<?,?> identifiableItem) {
            ...
            return (KeyedInstanceIdentifier<T, K>) KeyedInstanceIdentifier.trustedCreate(arg,
                    pathArguments, hashBuilder.build(), true,
                    (K) identifiableItem.getKey());
        } else {
            throw new IllegalStateException("Arg is not IdentifiableItem");
        }
    }
