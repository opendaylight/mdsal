 ...
 */
public record KeyPredicateStep<K extends Key<T>, T extends KeyAware<K> & DataObject>(
    ...
    public KeyPredicateStep {
        NodeIdentifierStep.checkType(type, true);
        NodeIdentifierStep.checkCaseType(caseType);
        requireNonNull(key);
    }

    public KeyPredicateStep(final @NonNull Class<T> type, final @NonNull K key) {
        this(type, null, key);
    }
}
