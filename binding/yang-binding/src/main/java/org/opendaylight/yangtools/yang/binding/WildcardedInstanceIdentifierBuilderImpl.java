...
public class WildcardedInstanceIdentifierBuilderImpl<T extends DataObject> extends
    ...
    WildcardedInstanceIdentifierBuilderImpl(final PathArgument item,
            final Iterable<? extends PathArgument> pathArguments, final int hash,
            final ImmutableList.@Nullable Builder<PathArgument> pathBuilder) {
        super(item, pathArguments, hash, pathBuilder);
    }

    WildcardedInstanceIdentifierBuilderImpl(final PathArgument item,
            final Iterable<? extends PathArgument> pathArguments,
            final int hash) {
    ...
    WildcardedInstanceIdentifierBuilderImpl<N> child(Class<@NonNull N> listItem, K listKey) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public @NonNull <N extends DataObject & Augmentation<? super T>>
        WildcardedInstanceIdentifierBuilderImpl<N> augmentation(
            Class<N> container) {
        Preconditions.checkArgument(Identifiable.class.isAssignableFrom(container),
                "Must not be Identifiable. Use wildcardChild() method instead");
        addNode(Item.of(container));
