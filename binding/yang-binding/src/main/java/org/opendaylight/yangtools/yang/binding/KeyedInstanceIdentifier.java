...
public final class KeyedInstanceIdentifier<T extends KeyAware<K> & DataObject, K extends Key<T>>
    ...
    private final @NonNull KeyPredicateStep<K, T> lastStep;

    KeyedInstanceIdentifier(final KeyPredicateStep<K, T> lastStep, final Iterable<DataObjectStep<?>> pathArguments,
            final boolean wildcarded, final int hash) {
        super(lastStep.type(), pathArguments, wildcarded, hash);
        this.lastStep = lastStep;
    }

    @NonNull KeyPredicateStep<K, T> lastStep() {
        return lastStep;
    }
