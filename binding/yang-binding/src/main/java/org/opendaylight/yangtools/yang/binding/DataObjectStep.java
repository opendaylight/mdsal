...
public sealed interface DataObjectStep<T extends DataObject> extends Comparable<DataObjectStep<?>>, Serializable
    ...
    private static int compareHierarchy(final DataObjectStep<?> recv, final DataObjectStep<?> other) {
        if (recv instanceof NodeIdentifierStep) {
            if (other instanceof NodeIdentifierStep) {
                return 0;
            } else if (other instanceof KeylessStep || other instanceof KeyPredicateStep) {
                return -1;
            }
        } else if (recv instanceof KeyPredicateStep thisAware) {
            if (other instanceof KeyPredicateStep otherAware) {
                @SuppressWarnings("unchecked")
                final var thisKey = (Comparable<Object>) thisAware.key();
                return  thisKey.compareTo(otherAware.key());
            } else if (other instanceof NodeIdentifierStep || other instanceof KeylessStep) {
                return 1;
            }
        } else if (recv instanceof KeylessStep) {
            if (other instanceof KeylessStep) {
                return 0;
            } else if (other instanceof NodeIdentifierStep || other instanceof KeyPredicateStep) {
                return 1;
            }
        }
