...
public sealed class InstanceIdentifier<T extends DataObject>
    ...
    public final <N extends KeyAware<K> & DataObject, K extends Key<N>> @Nullable K firstKeyOf(
        ...
        for (var step : pathArguments) {
            if (step instanceof KeyPredicateStep<?, ?> keyPredicate && listItem.equals(step.type())) {
                @SuppressWarnings("unchecked")
    ...
    public final <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>>
        ...
        return (KeyedInstanceIdentifier<N, K>) childIdentifier(new KeyPredicateStep<>(listItem, listKey));
    }
    ...
    public final <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>,
        ...
        return (KeyedInstanceIdentifier<N, K>) childIdentifier(
            new KeyPredicateStep<>(listItem, requireNonNull(caze), listKey));
    }
    ...
    public final <N extends DataObject & Augmentation<? super T>> @NonNull InstanceIdentifier<N> augmentation(
            final Class<@NonNull N> container) {
        return childIdentifier(new NodeIdentifierStep<>(container));
    }
    ...
    public static <N extends KeyAware<K> & ChildOf<? extends DataRoot>,
            K extends Key<N>> @NonNull KeyedBuilder<N, K> builder(final Class<N> listItem,
                    final K listKey) {
        return new KeyedBuilder<>(new KeyPredicateStep<>(listItem, listKey));
    }
    ...
    public static <C extends ChoiceIn<? extends DataRoot> & DataObject,
            ...
            @NonNull KeyedBuilder<N, K> builder(final Class<C> caze, final Class<N> listItem,
                    final K listKey) {
        return new KeyedBuilder<>(new KeyPredicateStep<>(listItem, requireNonNull(caze), listKey));
    }
    ...
    private static <T extends DataObject, C extends ChoiceIn<?> & DataObject> DataObjectStep<T> createStep(
            final Class<C> caze, final Class<T> type) {
        return KeyAware.class.isAssignableFrom(type) ? new KeylessStep(type, caze)
            : new NodeIdentifierStep<>(type, caze);
    }

    private static <T extends DataObject> DataObjectStep<T> createStep(final Class<T> type) {
    ...
    static <N extends DataObject> @NonNull InstanceIdentifier<N> trustedCreate(final DataObjectStep<?> lastStep,
        ...
        if (lastStep instanceof NodeIdentifierStep) {
            return new InstanceIdentifier(lastStep.type(), pathArguments, wildcarded, hash);
        } else if (lastStep instanceof KeyPredicateStep<?, ?> predicate) {
     ...
     */
    private interface PathArgument extends Comparable<PathArgument> {
        /**
    ...
    abstract static sealed class AbstractPathArgument<T extends DataObject> implements PathArgument, Serializable {
        @java.io.Serial
        ...
        abstract @NonNull DataObjectStep<T> toStep();
    }
     ...
     */
    @Deprecated(since = "13.0.0", forRemoval = true)
    static sealed class Item<T extends DataObject> extends AbstractPathArgument<T> {
        @java.io.Serial
        private static final long serialVersionUID = 1L;
        ...
        @Override
        DataObjectStep<T> toStep() {
            final var type = getType();
            final var caseType = getCaseType().orElse(null);
            @SuppressWarnings({ "rawtypes", "unchecked" })
            final DataObjectStep<T> ret = KeyAware.class.isAssignableFrom(type) ? new KeylessStep(type, caseType)
                : new NodeIdentifierStep<>(type, caseType);
            return ret;
        }
    }
     ...
     */
    @Deprecated(since = "13.0.0", forRemoval = true)
    static sealed class IdentifiableItem<I extends KeyAware<T> & DataObject, T extends Key<I>>
        ...
        @Override
        final KeyPredicateStep<T, I> toStep() {
            return new KeyPredicateStep<>(getType(), getCaseType().orElse(null), key);
        }
    }
    ...
    public abstract static sealed class Builder<T extends DataObject> {
        ...
        public final <N extends DataObject & Augmentation<? super T>> Builder<N> augmentation(
            ...
            return append(new NodeIdentifierStep<>(container));
        }
        ...
        public final <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>> KeyedBuilder<N, K> child(
            ...
            return append(new KeyPredicateStep<>(listItem, listKey));
        }
        ...
        public final <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>,
                N extends KeyAware<K> & ChildOf<? super C>> KeyedBuilder<N, K> child(final Class<C> caze,
                    final Class<N> listItem, final K listKey) {
            return append(new KeyPredicateStep<>(listItem, requireNonNull(caze), listKey));
        }
        ...
        abstract <X extends DataObject & KeyAware<Y>, Y extends Key<X>> @NonNull KeyedBuilder<X, Y> append(
            KeyPredicateStep<Y, X> step);
    }

    public static final class KeyedBuilder<T extends DataObject & KeyAware<K>, K extends Key<T>>
            extends Builder<T> {
        private @NonNull KeyPredicateStep<K, T> lastStep;

        KeyedBuilder(final KeyPredicateStep<K, T> firstStep) {
        ...
        private KeyedBuilder(final RegularBuilder<?> prev, final KeyPredicateStep<K, T> lastStep) {
            super(prev, lastStep);
        ...
        <X extends DataObject & KeyAware<Y>, Y extends Key<X>> KeyedBuilder<X, Y> append(
                final KeyPredicateStep<Y, X> step) {
            appendItem(step);
            lastStep = (KeyPredicateStep<K, T>) requireNonNull(step);
    ...
    private static final class RegularBuilder<T extends DataObject> extends Builder<T> {
        ...
        @Override
        <X extends DataObject & KeyAware<Y>, Y extends Key<X>> KeyedBuilder<X, Y> append(
                final KeyPredicateStep<Y, X> item) {
