...
public sealed class InstanceIdentifier<T extends DataObject>
     ...
     */
    final Iterable<ContainerStep<?>> pathArguments;

    private final @NonNull Class<T> targetType;
    private final boolean wildcarded;
    private final int hash;

    InstanceIdentifier(final Class<T> type, final Iterable<ContainerStep<?>> pathArguments, final boolean wildcarded,
            final int hash) {
     ...
     */
    public final @NonNull Iterable<ContainerStep<?>> getPathArguments() {
    ...
    public final boolean containsWildcarded(final InstanceIdentifier<?> other) {
        ...
        for (var step : pathArguments) {
            ...
            if (step instanceof ExactContainerStep) {
    ...
    private <N extends DataObject> @NonNull InstanceIdentifier<N> childIdentifier(final ContainerStep<N> arg) {
    ...
    private static <T extends DataObject, C extends ChoiceIn<?> & DataObject> @NonNull ContainerStep<T> createStep(
    ...
    private static <T extends DataObject> @NonNull ContainerStep<T> createStep(final Class<T> type) {
     ...
     */
    private static @NonNull InstanceIdentifier<?> internalCreate(final Iterable<ContainerStep<?>> pathArguments) {
        ...
        final var hashBuilder = new HashCodeBuilder<ContainerStep<?>>();
        ...
        do {
            ...
            if (!(arg instanceof ExactContainerStep)) {
                wildcard = true;
            }
        } while (it.hasNext());
    ...
    /**
     * Create an instance identifier for a sequence of {@link ContainerStep} steps. The steps are required to be formed
    ...
    public static <T extends DataObject> @NonNull InstanceIdentifier<T> unsafeOf(
            final List<? extends ContainerStep<?>> pathArguments) {
    ...
    static <N extends DataObject> @NonNull InstanceIdentifier<N> trustedCreate(final ContainerStep<?> lastStep,
            final Iterable<ContainerStep<?>> pathArguments, final int hash, final boolean wildcarded) {
    ...
    public abstract static sealed class Builder<T extends DataObject> {
        private final ImmutableList.Builder<ContainerStep<?>> pathBuilder;
        private final HashCodeBuilder<ContainerStep<?>> hashBuilder;
        private final Iterable<? extends ContainerStep<?>> basePath;

        private boolean wildcard;

        Builder(final Builder<?> prev, final ContainerStep<?> item) {
        ...
        Builder(final ContainerStep<?> item, final boolean wildcard) {
            pathBuilder = ImmutableList.builder();
        ...
        final void appendItem(final ContainerStep<?> item) {
            hashBuilder.addArgument(item);
            pathBuilder.add(item);
            if (!(item instanceof ExactContainerStep)) {
                wildcard = true;
            }
        }

        abstract <X extends DataObject> @NonNull RegularBuilder<X> append(ContainerStep<X> step);
    ...
    public static final class KeyedBuilder<T extends DataObject & KeyAware<K>, K extends Key<T>>
        ...
        @Override
        <X extends DataObject> @NonNull RegularBuilder<X> append(final ContainerStep<X> step) {
    ...
    private static final class RegularBuilder<T extends DataObject> extends Builder<T> {
        ...
        RegularBuilder(final ContainerStep<T> item) {
            super(item, !(item instanceof ExactContainerStep));
            type = item.type();
        }
        ...
        private RegularBuilder(final KeyedBuilder<?, ?> prev, final ContainerStep<T> item) {
            super(prev, item);
            type = item.type();
        }
        ...
        @Override
        @SuppressWarnings({ "rawtypes", "unchecked" })
        <X extends DataObject> RegularBuilder<X> append(final ContainerStep<X> step) {
            appendItem(step);
