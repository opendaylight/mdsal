...
import org.opendaylight.mdsal.binding.testkit.spi.AbstractBindingServiceTestKit;
...
import org.opendaylight.mdsal.dom.testkit.spi.AbstractDOMServiceTestKit.ListenerPolicy.Capture;
 ...
 */
@Beta
public class BindingServiceTestKit extends AbstractBindingServiceTestKit<BindingDOMDataBrokerAdapter> {

    public BindingServiceTestKit() {
        this(new DOMServiceTestKit());
    }

    public BindingServiceTestKit(final DOMServiceTestKit domTestKit) {
        super(domTestKit);
    }

    public BindingServiceTestKit(final DOMServiceTestKit domTestKit, final TreeChangeListenerClassifier classifier) {
        super(domTestKit, classifier);
    }

    private final Queue<CapturedDataTreeChangeEvent<?>> events = new ArrayDeque<>();

    protected <T extends DataObject> @NonNull ListenerPolicy dtclPolicyFor(final DataTreeIdentifier<T> treeId,
            final DataTreeChangeListener<T> listener) {
        return ListenerPolicy.PASS;
    }
    ...
    private final class TestingDataBroker extends ForwardingDataBroker {
        ...
        public <T extends DataObject, L extends DataTreeChangeListener<T>> ListenerRegistration<L>
            ...
            final ListenerPolicy policy = dtclPolicyFor(treeId, listener);
            if (ListenerPolicy.PASS.equals(policy)) {
                ...
                return super.registerDataTreeChangeListener(treeId, listener);
            } else if (policy instanceof Capture) {
    ...
    protected BindingDOMDataBrokerAdapter createDataBroker() {
        return new BindingDOMDataBrokerAdapter(domTestKit().domDataBroker(), domCodec());
    }
