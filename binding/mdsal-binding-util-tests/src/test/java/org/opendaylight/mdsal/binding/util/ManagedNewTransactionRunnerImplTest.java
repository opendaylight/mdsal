...
public class ManagedNewTransactionRunnerImplTest extends AbstractConcurrentDataBrokerTest {
    ...
    public void testApplyWithNewReadTransactionAndCloseEmptySuccessfully() throws Exception {
        assertEquals(Long.valueOf(1),
            managedNewTransactionRunner.applyWithNewReadOnlyTransactionAndClose(OPERATIONAL,
                (CheckedFunction<TypedReadTransaction<Datastore.Operational>, Long, RuntimeException>) tx -> 1L));
    }

    @Test
    public void testCallWithNewReadTransactionAndCloseEmptySuccessfully() throws Exception {
        managedNewTransactionRunner.callWithNewReadOnlyTransactionAndClose(OPERATIONAL,
            (CheckedConsumer<TypedReadTransaction<Datastore.Operational>, RuntimeException>) tx -> { });
    }
    ...
    public void testCallWithNewReadTransactionAndCloseReadSuccessfully() throws Exception {
        ...
        assertEquals(data, managedNewTransactionRunner.applyWithNewReadOnlyTransactionAndClose(OPERATIONAL,
            (CheckedFunction<TypedReadTransaction<Datastore.Operational>, FluentFuture<Optional<TopLevelList>>,
                RuntimeException>) tx -> tx.read(TEST_PATH)).get().get());
    }
