...
/**
 * A composite node in generator tree. Composite nodes may contain additional children, which end up being mapped into
 * the naming hierarchy 'under' the composite node. To support this use case, each composite has a Java package name
 * assigned.
 */
public abstract class AbstractCompositeGeneratorNode<T extends EffectiveStatement<?, ?>>
        extends AbstractGeneratorNode<T> {
    private final List<AbstractGeneratorNode<?>> children;
    private final List<UsesEffectiveStatement> uses;

    private String javaPackage;

    AbstractCompositeGeneratorNode(final T statement) {
        super(statement);

        final List<UsesEffectiveStatement> tmpUses = new ArrayList<>(2);
        final List<AbstractGeneratorNode<?>> tmpChildren = new ArrayList<>();
        ...
        for (EffectiveStatement<?, ?> stmt : statement.effectiveSubstatements()) {
            ...
            final AbstractGeneratorNode<?> child;
            if (stmt instanceof ActionEffectiveStatement) {
                child = new ActionGeneratorNode((ActionEffectiveStatement) stmt);
            } else if (stmt instanceof AnydataEffectiveStatement) {
                child = new AnydataGeneratorNode((AnydataEffectiveStatement) stmt);
            } else if (stmt instanceof AnyxmlEffectiveStatement) {
                child = new AnyxmlGeneratorNode((AnyxmlEffectiveStatement) stmt);
            } else if (stmt instanceof AugmentEffectiveStatement) {
                child = new AugmentGeneratorNode((AugmentEffectiveStatement) stmt);
            } else if (stmt instanceof CaseEffectiveStatement) {
                child = new CaseGeneratorNode((CaseEffectiveStatement) stmt);
            } else if (stmt instanceof ChoiceEffectiveStatement) {
                child = new ChoiceGeneratorNode((ChoiceEffectiveStatement) stmt);
            } else if (stmt instanceof ContainerEffectiveStatement) {
                child = new ContainerGeneratorNode((ContainerEffectiveStatement) stmt);
            } else if (stmt instanceof GroupingEffectiveStatement) {
                child = new GroupingGeneratorNode((GroupingEffectiveStatement) stmt);
            } else if (stmt instanceof IdentityEffectiveStatement) {
                child = new IdentityGeneratorNode((IdentityEffectiveStatement) stmt);
            } else if (stmt instanceof InputEffectiveStatement) {
                child = new InputGeneratorNode((InputEffectiveStatement) stmt);
            } else if (stmt instanceof LeafEffectiveStatement) {
                child = new LeafGeneratorNode((LeafEffectiveStatement) stmt);
            } else if (stmt instanceof LeafListEffectiveStatement) {
                child = new LeafListGeneratorNode((LeafListEffectiveStatement) stmt);
            } else if (stmt instanceof ListEffectiveStatement) {
                child = new ListGeneratorNode((ListEffectiveStatement) stmt);
            } else if (stmt instanceof NotificationEffectiveStatement) {
                child = new NotificationGeneratorNode((NotificationEffectiveStatement) stmt);
            } else if (stmt instanceof OutputEffectiveStatement) {
                child = new OutputGeneratorNode((OutputEffectiveStatement) stmt);
            } else if (stmt instanceof RpcEffectiveStatement) {
                child = new RpcGeneratorNode((RpcEffectiveStatement) stmt);
            } else if (stmt instanceof TypedefEffectiveStatement) {
                child = new TypedefGeneratorNode((TypedefEffectiveStatement) stmt);
            } else {
    ...
    final void assignChildNames() {
         ...
         */
        final Multimap<String, AbstractGeneratorNode<?>> toAssign = ArrayListMultimap.create();
        for (AbstractGeneratorNode<?> child : children) {
            toAssign.put(child.preferredName(), child);
        }

        // FIXME: do we need this map?
        final Map<String, AbstractGeneratorNode<?>> assignments = Maps.newHashMapWithExpectedSize(toAssign.size());
    ...
    // conflicts.
    private static void assignNonConflicting(final Multimap<String, AbstractGeneratorNode<?>> toAssign,
            final Map<String, AbstractGeneratorNode<?>> assignments) {
        final var it = toAssign.asMap().entrySet().iterator();
        while (it.hasNext()) {
            final Entry<String, Collection<AbstractGeneratorNode<?>>> entry = it.next();
            final Collection<AbstractGeneratorNode<?>> assignees = entry.getValue();
            if (assignees.size() == 1) {
                verify(assignees instanceof List, "Unexpected assignees %s", assignees);
                assign(assignments, ((List<AbstractGeneratorNode<?>>) assignees).get(0), entry.getKey());
                it.remove();
            }
        }
    }
    ...
    private static void assignConflicting(final Multimap<String, AbstractGeneratorNode<?>> toAssign,
            final Map<String, AbstractGeneratorNode<?>> assignments) {
        // Strategy one: assign a unique suffix to each name based on the YANG namespace it comes from. For that we need
        //               an index first...
        final Table<YangStatementNamespace, String, List<AbstractGeneratorNode<?>>> nsNameAssignee =
            HashBasedTable.create();
        for (Entry<String, Collection<AbstractGeneratorNode<?>>> entry : toAssign.asMap().entrySet()) {
            final var namespaceToChild = nsNameAssignee.column(entry.getKey());
            for (AbstractGeneratorNode<?> child : entry.getValue()) {
        ...
        final Multimap<String, AbstractGeneratorNode<?>> nsNameConflicts = ArrayListMultimap.create();
        for (var cell : nsNameAssignee.cellSet()) {
            final String assignedName = verifyNotNull(cell.getRowKey()).appendSuffix(cell.getColumnKey());
            final List<AbstractGeneratorNode<?>> assignees = verifyNotNull(cell.getValue());
            if (assignees.size() == 1) {
    ...
    // At this point we have taken YANG namespaces out of the picture, yet we still have some conflicts. At this point
    // the only reason for this can be that localName -> preferredName mapping is losing the difference.
    private static void assignNamespaceConflicting(final Multimap<String, AbstractGeneratorNode<?>> toAssign,
            final Map<String, AbstractGeneratorNode<?>> assignments) {
    ...
    private static void assign(final Map<String, AbstractGeneratorNode<?>> assignments,
            final AbstractGeneratorNode<?> assignee, final @NonNull String simpleName) {
