...
public final class ModuleGenerator extends AbstractCompositeGenerator<ModuleEffectiveStatement> {
    ...
    @Override
    Member createMember() {
        return domain().addPrimary(new ModuleNamingStrategy(statement().localQNameModule(), statement().argument()));
    }

    @Override
    boolean producesType() {
        for (Generator child : this) {
            if (child.producesType() && child instanceof AbstractExplicitGenerator
