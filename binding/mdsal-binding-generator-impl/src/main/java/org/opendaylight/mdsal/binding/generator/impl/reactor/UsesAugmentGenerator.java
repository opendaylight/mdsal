...
final class UsesAugmentGenerator extends AbstractAugmentGenerator {
    ...
    void loadTargetGenerator() {
         ...
         */
        final SchemaNodeIdentifier toSearch;
        final QNameModule grpModule = grp.statement().argument().getModule();
        if (!path.firstNodeIdentifier().getModule().equals(grpModule)) {
            toSearch = Descendant.of(path.getNodeIdentifiers().stream()
                .map(qname -> qname.bindTo(grpModule))
                .collect(ImmutableList.toImmutableList()));
        } else {
            toSearch = path;
        }

        setTargetGenerator(grp.resolveSchemaNode(toSearch));
    }
}
