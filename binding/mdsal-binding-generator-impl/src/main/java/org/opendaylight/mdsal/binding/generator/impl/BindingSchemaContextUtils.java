...
public final class BindingSchemaContextUtils {
    ...
    private static DataNodeContainer findInCases(final ChoiceSchemaNode choiceNode, final QName targetQName) {
        for (ChoiceCaseNode caze : choiceNode.getCases().values()) {
    ...
    public static Optional<ChoiceCaseNode> findInstantiatedCase(final ChoiceSchemaNode instantiatedChoice,
            final ChoiceCaseNode originalDefinition) {
        ...
        //
        for (ChoiceCaseNode caze : instantiatedChoice.findCaseNodes(originalDefinition.getQName().getLocalName())) {
