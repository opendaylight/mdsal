...
public final class GeneratorReactor extends GeneratorContext implements Mutable {
    ...
    private final @NonNull Collection<ModuleGenerator> children;
    ...
    public @NonNull Map<QNameModule, ModuleGenerator> execute(final TypeBuilderFactory builderFactory) {
        ...
        // ...  and also link augments and their targets in a separate pass, as we need groupings fully resolved before
        // we attempt augmentation lookups. There is a slight twist here, as we need to be mindful of the order in which
        // we process the augments.
        final List<AugmentGenerator> augments = new ArrayList<>();
        for (ModuleGenerator module : children) {
            for (Generator child : module) {
                if (child instanceof AugmentGenerator) {
                    augments.add((AugmentGenerator) child);
                }
            }
        }
        augments.sort(AugmentGenerator.COMPARATOR);
        augments.forEach(augment -> augment.linkAugmentationTarget(this));
    ...
    <E extends SchemaTreeEffectiveStatement<?>> AbstractExplicitGenerator<E> resolveSchemaNode(
        ...
        for (QName qname : path.getNodeIdentifiers()) {
            current = verifyNotNull(current.findSchemaTreeGenerator(qname), "Failed to find %s in %s", qname, current);
        }
