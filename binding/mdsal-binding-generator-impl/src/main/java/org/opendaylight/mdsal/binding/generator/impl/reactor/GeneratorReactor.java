...
public final class GeneratorReactor extends GeneratorContext implements Mutable {
    ...
    public @NonNull Map<QNameModule, ModuleGenerator> execute() {
        ...
        forEachSimpleGenerator(children, TypedefGenerator.class, TypedefGenerator::linkBaseGenerator);

        // Step two: link all type statements, so that leafs and leaf-lists have restrictions established
        forEachSimpleGenerator(children, AbstractTypeAwareGenerator.class, AbstractTypeAwareGenerator::linkType);

        // Step three: resolve all 'type leafref' statements, so they point to their corresponding Java type
        //             representation
        forEachSimpleGenerator(children, AbstractLeafrefAwareGenerator.class,
    ...
    private <T extends Generator<?>> void forEachSimpleGenerator(final Iterable<? extends Generator<?>> parent,
            final Class<T> type, final BiConsumer<T, GeneratorContext> action) {
        stack.push(parent);
        for (Generator<?> child : parent) {
            if (type.isInstance(child)) {
                action.accept(type.cast(child), this);
            } else if (child instanceof AbstractCompositeGenerator) {
                forEachSimpleGenerator((AbstractCompositeGenerator<?>) child, type, action);
            }
        }
        stack.pop();
    }
