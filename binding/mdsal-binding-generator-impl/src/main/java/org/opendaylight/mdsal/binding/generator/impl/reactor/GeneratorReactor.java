...
public final class GeneratorReactor extends GeneratorContext implements Mutable {
    ...
    public @NonNull Map<QNameModule, ModuleGenerator> execute(final TypeBuilderFactory builderFactory) {
        ...
        forEachCompositeGenerator(AbstractCompositeGenerator::linkCompositeDependencies);
        ...
        // FIXME: augmentations are throwing a nasty here. In order to find the augmentation name, we need its target's
        //        name first. For that to work, we need to assign simple names depth-first, excluding modules.
