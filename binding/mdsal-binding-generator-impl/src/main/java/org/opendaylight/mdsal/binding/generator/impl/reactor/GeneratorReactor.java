...
public final class GeneratorReactor extends GeneratorContext implements Mutable {
    ...
    public @NonNull Map<QNameModule, ModuleGenerator> execute(final TypeBuilderFactory builderFactory) {
         ...
         *
         * Since our implementation class hierarchy captures all three statements involved in a common superclass, we
         * can perform this in a single pass.
         */
        forEachTypeObjectGenerator(children, AbstractTypeObjectGenerator::linkType);

        // Step three: resolve all 'type leafref' and 'type identityref' statements, so they point to their
        ...
        // Step five: walk all composite generators and link ChildOf/ChoiceIn relationships with parents. We have taken
        //            care of this step during tree construction, hence this now a no-op.

        /*
         * Step six: assign java packages and JavaTypeNames
         *
        ...
        // First we'll assign simple names, recursively for each module. These are to be used for the purposes of
        // establishing top-level class names. We then assign package names based on those names for the use by their
        // children.
    ...
    IdentityGenerator resolveIdentity(final QName name) {
        ...
        if (module != null) {
            for (Generator<?> gen : module) {
                if (gen instanceof IdentityGenerator && name.equals(gen.statement().argument())) {
                    return (IdentityGenerator) gen;
                }
            }
        }
    ...
    private @NonNull AbstractTypeObjectGenerator<?> mapToGenerator() {
        ...
        Generator<?> current = verifyNotNull(generators.get(module.localQNameModule()), "Cannot match module %s",
            module);
        verify(module == current.statement(), "Module mismatch on %s", module);
