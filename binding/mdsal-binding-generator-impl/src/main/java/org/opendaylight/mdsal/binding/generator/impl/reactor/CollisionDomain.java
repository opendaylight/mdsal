...
final class CollisionDomain {
    ...
    private final class Secondary extends Member {
        ...
        Secondary(final Primary primary, final String classSuffix, final String packageSuffix) {
        ...
        String computeCurrentClass() {
            return primary.currentClass() + classSuffix;
        }

        @Override
        String computeCurrentPackage() {
            final String primaryPackage = primary.currentPackage();
            final String suffix = verifyNotNull(packageSuffix);
            return suffix.isEmpty() ? primaryPackage : primaryPackage + '.' + suffix;
        }

        @Override
        boolean signalConflict() {
            return primary.signalConflict();
        }
    ...
    @NonNull Member addSecondary(final Member primary, final String classSuffix, final @Nullable String packageSuffix) {
        verify(primary instanceof Primary, "Unexpected primary %s", primary);
        final Primary cast = (Primary) primary;
        final Secondary secondary = new Secondary(cast, classSuffix, packageSuffix);
