...
abstract class AbstractExplicitGenerator<T extends EffectiveStatement<?, ?>> extends Generator implements CopyableNode {
    ...
    final Member ensureMember() {
        if (member == null) {
            ...
            switch (placement) {
                ...
                case PHANTOM:
                case TOP_LEVEL:
                    member = Optional.of(createMember());
                    break;
                default:
    ...
    @NonNull Member createMember() {
        return getParent().domain().addPrimary(new CamelCaseNamingStrategy(namespace(), localName()));
    }

    @Override
    ToStringHelper addToStringAttributes(final ToStringHelper helper) {
        return helper.add("argument", statement.argument());
    }
}
