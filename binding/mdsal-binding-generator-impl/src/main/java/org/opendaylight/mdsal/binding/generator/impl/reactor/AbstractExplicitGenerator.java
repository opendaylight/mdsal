...
public abstract class AbstractExplicitGenerator<T extends EffectiveStatement<?, ?>> extends Generator
    ...
    // FIXME: this is usually a proxy to original. We probably want to use a dedicated class for this
    ...
    final void linkOriginalGenerator(final GeneratorContext context) {
        if (isAddedByUses()) {
            LOG.info("Linking {}", this);
            prev = getParent().getOriginalChild(getQName());
            LOG.info("Linked {} to {}", this, prev);
        }
    }
    ...
    final @NonNull AbstractExplicitGenerator<?> resolveSchemaNode(final @NonNull SchemaNodeIdentifier path) {
        ...
        // FIXME: does not quite work as we need to find the original target, i.e. we need to understand when we are
        //        following into an augmentation/grouping so that we correctly adjust remaining QNames to match
