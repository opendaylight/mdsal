...
abstract class AbstractTypeGenerator {
    ...
    private GeneratedTypeBuilder processDataSchemaNode(final ModuleContext context, final Type baseInterface,
        ...
        overrideDataObjectMethods(genType, true);
    ...
    private Type createRpcContainer(final ModuleContext context, final String rpcName, final RpcDefinition rpc,
        ...
        overrideDataObjectMethods(outType, true);
    ...
    private void notificationsToGenType(final ModuleContext context) {
        ...
        for (final NotificationDefinition notification : notifications) {
            if (notification != null) {
                ...
                overrideDataObjectMethods(notificationInterface, true);
    ...
    private void groupingsToGenTypes(final ModuleContext context, final Collection<GroupingDefinition> groupings) {
        for (final GroupingDefinition grouping : new GroupingDefinitionDependencySort().sort(groupings)) {
            ...
            overrideDataObjectMethods(genType, false);
    ...
    private GeneratedTypeBuilder addRawAugmentGenTypeDefinition(final ModuleContext context,
        ...
        overrideDataObjectMethods(augTypeBuilder, true);
    ...
    private void generateTypesFromChoiceCases(final ModuleContext context, final Type refChoiceType,
        ...
        for (final CaseSchemaNode caseNode : choiceNode.getCases().values()) {
            if (caseNode != null && !caseNode.isAddedByUses() && !caseNode.isAugmenting()) {
                ...
                overrideDataObjectMethods(caseTypeBuilder, true);
    ...
    private static void overrideDataObjectMethods(final GeneratedTypeBuilder typeBuilder, final boolean isDefault) {
        final JavaTypeName typeName = typeBuilder.getIdentifier();
        typeBuilder.addMethod(BindingMapping.DATA_CONTAINER_IMPLEMENTED_INTERFACE_NAME)
        .setAccessModifier(AccessModifier.PUBLIC).setDefault(isDefault)
        .setReturnType(classType(isDefault ? wildcardTypeFor(typeName) : new ReferencedTypeImpl(typeName)))
        .addAnnotation(OVERRIDE_ANNOTATION);
    }
}
