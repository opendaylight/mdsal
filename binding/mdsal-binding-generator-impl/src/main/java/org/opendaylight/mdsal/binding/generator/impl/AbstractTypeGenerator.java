...
abstract class AbstractTypeGenerator {
    ...
    private static InheritedGetter findInheritedGetter(final String localName, final GeneratedType impl) {
        ...
        final String getterName = BindingMapping.getGetterMethodName(nameRoot, false);
        final String booleanGetterName = BindingMapping.getGetterMethodName(nameRoot, true);
        return findInheritedGetter(impl, getterName, booleanGetterName);
    }

    private static InheritedGetter findInheritedGetter(final GeneratedType type, final String getterName,
            final String booleanGetterName) {
        for (MethodSignature method : type.getMethodDefinitions()) {
            if (booleanGetterName.equals(method.getName())) {
                return InheritedGetter.RESOLVED;
            }
        ...
        for (Type implementsType : type.getImplements()) {
            if (implementsType instanceof GeneratedType) {
                final InheritedGetter getter = findInheritedGetter((GeneratedType) implementsType, getterName,
                    booleanGetterName);
                if (getter != InheritedGetter.NOT_PRESENT) {
                    return getter;
                }
            }
        }
