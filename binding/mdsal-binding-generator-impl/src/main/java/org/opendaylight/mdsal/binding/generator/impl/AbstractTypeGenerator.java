...
abstract class AbstractTypeGenerator {
    ...
    private void containerToGenType(final ModuleContext context, final GeneratedTypeBuilder parent,
        ...
        if (genType != null) {
            ...
            actionsToGenType(context, node.getActions());
        }
    }
    ...
    private <T extends DataSchemaNode & ActionNodeContainer> void actionsToGenType(final ModuleContext context,
        ...
        for (final ActionDefinition action : actions) {
            ...
            builder.addImplementsType(BindingTypes.action(BindingTypes.instanceIdentifier(parent),
                createOperationContainer(context, operName, action, verifyNotNull(action.getInput())),
    ...
    private void rpcMethodsToGenType(final ModuleContext context) {
        ...
        for (final RpcDefinition rpc : rpcDefinitions) {
            ...
            method.setReturnType(Types.parameterizedTypeFor(FUTURE,
                rpcResult(createOperationContainer(context, rpcName, rpc, verifyNotNull(rpc.getOutput())))));
        }
    ...
    private Type createOperationContainer(final ModuleContext context, final String operName,
        ...
        final GeneratedTypeBuilder outType = addRawInterfaceDefinition(JavaTypeName.create(
            BindingGeneratorUtil.packageNameForGeneratedType(context.modulePackageName(), oper.getPath()),
            operName + BindingMapping.getClassName(schema.getQName())),
            schema);
