...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends AbstractExplicitGenerator<T> {
    ...
    final @NonNull AbstractExplicitGenerator<?> getOriginalChild(final QName childQName) {
        ...
        for (GroupingGenerator grouping : groupings) {
            final QName qname = childQName.bindTo(grouping.statement().argument().getModule());
            final AbstractExplicitGenerator<?> gen = grouping.findSchemaTreeGenerator(qname);
            if (gen != null) {
                return gen.getOriginal();
            }
        }
        // ... next try local augments...
        for (AbstractAugmentGenerator augment : augments) {
            final AbstractExplicitGenerator<?> gen = augment.findSchemaTreeGenerator(childQName);
            if (gen != null) {
                return gen.getOriginal();
            }
        }
