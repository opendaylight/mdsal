 ...
 * name assigned.
 */
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends Generator<T> {
    private final List<Generator<?>> children;

    private List<GroupingGenerator> usedGroupings;
    private AbstractCompositeGenerator<?> parent;
    ...
    AbstractCompositeGenerator(final T statement) {
        ...
        for (EffectiveStatement<?, ?> stmt : statement.effectiveSubstatements()) {
            ...
            if (stmt instanceof ActionEffectiveStatement) {
                ...
                child = new OpaqueObjectGenerator<>((AnydataEffectiveStatement) stmt);
            } else if (stmt instanceof AnyxmlEffectiveStatement) {
                child = new OpaqueObjectGenerator<>((AnyxmlEffectiveStatement) stmt);
    ...
    final void linkUses(final GeneratorContext context) {
    ...
    final void linkParent(final AbstractCompositeGenerator<?> parentGenerator) {
        this.parent = requireNonNull(parentGenerator);
    }
