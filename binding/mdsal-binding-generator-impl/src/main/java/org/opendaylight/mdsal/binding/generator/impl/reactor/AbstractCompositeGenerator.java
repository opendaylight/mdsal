...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends Generator<T> {
    ...
    private List<Generator<?>> createChildren(final EffectiveStatement<?, ?> statement) {
        ...
        for (EffectiveStatement<?, ?> stmt : statement.effectiveSubstatements()) {
            ...
            if (stmt instanceof ActionEffectiveStatement) {
                ...
                child = new AugmentGenerator((AugmentEffectiveStatement) stmt, this);
    ...
    @Override
    final Generator<?> findGenerator(final EffectiveStatement<?, ?> stmt) {
        // TODO: use a map for children?
        for (Generator<?> gen : children) {
            if (gen.statement() == stmt) {
                return gen;
            }
        }
        return null;
        ...
        for (Generator<?> child : children) {
            if (child instanceof AbstractCompositeGenerator) {
                ...
                if (!composite.children.isEmpty()) {
                    child.setJavaPackage(BindingMapping.normalizePackageName(
                        myPackage + '.' + child.preferredSubpackage()));
                }
            }
        }
    }
