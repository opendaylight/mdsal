    ...
    private List<Generator> createChildren(final EffectiveStatement<?, ?> statement, final boolean includeAugments) {
        ...
        for (EffectiveStatement<?, ?> stmt : statement.effectiveSubstatements()) {
            if (stmt instanceof ActionEffectiveStatement) {
                final ActionGenerator actionGen = new ActionGenerator((ActionEffectiveStatement) stmt, this);
                tmp.add(actionGen);
                // FIXME: do not special-case these once we can generate new RPC layout
                tmp.add(new InputGenerator(
                    stmt.findFirstEffectiveSubstatement(InputEffectiveStatement.class).orElseThrow(), actionGen));
                tmp.add(new OutputGenerator(
                    stmt.findFirstEffectiveSubstatement(OutputEffectiveStatement.class).orElseThrow(), actionGen));
                ...
                final RpcGenerator rpcGen = new RpcGenerator((RpcEffectiveStatement) stmt, this);
                tmp.add(rpcGen);
                // FIXME: do not generate legacy RPC layout
                tmp.add(new InputGenerator.Rpc(
                    stmt.findFirstEffectiveSubstatement(InputEffectiveStatement.class).orElseThrow(), this, rpcGen));
                tmp.add(new OutputGenerator.Rpc(
                    stmt.findFirstEffectiveSubstatement(OutputEffectiveStatement.class).orElseThrow(), this, rpcGen));
