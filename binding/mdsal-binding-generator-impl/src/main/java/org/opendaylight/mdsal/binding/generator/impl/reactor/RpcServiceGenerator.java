...
final class RpcServiceGenerator extends AbstractImplicitGenerator {
    ...
    GeneratedType createTypeImpl(final TypeBuilderFactory builderFactory) {
        ...
        for (RpcGenerator rpcGen : rpcs) {
            ...
            method.addParameter(getChild(rpcGen, InputEffectiveStatement.class).getType(builderFactory), "input");
            method.setReturnType(Types.listenableFutureTypeFor(getChild(rpcGen, OutputEffectiveStatement.class)
                .getType(builderFactory)));
    ...
    private static <T extends EffectiveStatement<?, ?>> Generator getChild(final Generator rpc, final Class<T> type) {
        for (Generator child : rpc) {
            if (child instanceof AbstractExplicitGenerator
                && type.isInstance(((AbstractExplicitGenerator<?>) child).statement())) {
                return child;
            }
        }
        throw new IllegalStateException("Cannot find " + type + " in " + rpc);
    }
}
