...
public abstract class Generator<T extends EffectiveStatement<?, ?>> implements Iterable<Generator<?>> {
    ...
    final @NonNull GeneratedType createType(final TypeBuilderFactory builderFactory) {
        ...
        if (local == null) {
            type = local = createTypeImpl(builderFactory);
        }
        return local;
    }

    abstract @NonNull GeneratedType createTypeImpl(TypeBuilderFactory builderFactory);
    ...
    /**
     * Add a specialized {@link DataContainer#implementedInterface() implementation to specified builder.
     *
    ...
    static final void defaultImplementedInterace(final GeneratedTypeBuilder typeBuilder) {
