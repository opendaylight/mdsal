...
public abstract class Generator implements Iterable<Generator> {
    ...
    final void addImplementsChildOf(final GeneratedTypeBuilder builder) {
        ...
        while (true) {
            ...
            } else if (ancestor instanceof UsesAugmentGenerator) {
                // we have been augmented into a grouping, we need to follow back
                ancestor = ((UsesAugmentGenerator) ancestor).targetGenerator();
            } else {
                break;
            }
        }
    ...
    static final <T extends EffectiveStatement<?, ?>> Generator getChild(final Generator parent, final Class<T> type) {
