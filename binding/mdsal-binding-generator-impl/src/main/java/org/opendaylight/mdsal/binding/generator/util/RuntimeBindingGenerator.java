...
public final class RuntimeBindingGenerator {
    ...
    private void generateActionTypes(final Module module, final ActionNodeContainer actionNodeContainer) {
        actionNodeContainer.getActions().forEach(node -> generateOperationType(module,  node));
    }
    ...
    private void generateRpcTypes(final Module module) {
        module.getRpcs().forEach(node -> generateOperationType(module, node));
    }

    protected void generateOperationType(final Module module, final OperationDefinition operation) {
        generateTypedefTypes(module, operation.getTypeDefinitions());
        generateOperationArgType(module, operation, operation.getInput());
        generateOperationArgType(module, operation, operation.getOutput());
    }

    private void generateOperationArgType(final Module module, final OperationDefinition operation,
                                          final DataNodeContainer argNode) {
        final String packageName = packageNameForModule(module);

        final String typeName = new StringBuilder().append(operation.getQName().getLocalName()).append('_')
