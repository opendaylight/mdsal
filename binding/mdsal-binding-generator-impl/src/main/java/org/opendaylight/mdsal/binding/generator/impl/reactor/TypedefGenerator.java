...
//        addUnitsToGenTO(genTOBuilder, typedef.getUnits().orElse(null));
    ...
    GeneratedType createDerivedType(final TypeBuilderFactory builderFactory, final GeneratedType baseType) {
        ...
        // FIXME: this check is not right: it hides the fact we cannot correctly generated restricted enumerations
        if (baseType instanceof GeneratedTransferObject) {
            builder.setExtendsType((GeneratedTransferObject) baseType);
        }
