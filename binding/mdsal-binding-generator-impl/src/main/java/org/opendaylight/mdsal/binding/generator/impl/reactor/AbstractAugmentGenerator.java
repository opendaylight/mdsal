...
abstract class AbstractAugmentGenerator extends AbstractCompositeGenerator<AugmentEffectiveStatement> {
    ...
    // Exposed for CaseGenerator
    final @NonNull ChoiceGenerator targetChoiceGenerator() {
        final AbstractCompositeGenerator<?> target = targetGenerator();
        verify(target instanceof ChoiceGenerator, "Unexpected parent augment %s target %s", this, target);
        return (ChoiceGenerator) target;
    }
