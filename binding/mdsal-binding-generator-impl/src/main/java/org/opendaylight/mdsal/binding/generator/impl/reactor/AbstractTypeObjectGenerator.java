...
abstract class AbstractTypeObjectGenerator<T extends EffectiveStatement<?, ?>> extends AbstractDependentGenerator<T> {
    ...
    final GeneratedType createTypeImpl(final TypeBuilderFactory builderFactory) {
        ...
        if (TypeDefinitions.BITS.equals(arg)) {
            ...
            final GeneratedTransferObject ret = createUnion(tmp, builderFactory, statement(), typeName(),
                currentModule(), type, isTypedef);
...
//        addUnitsToGenTO(genTOBuilder, typedef.getUnits().orElse(null));
    ...
    private static @NonNull GeneratedTransferObject createUnion(final List<GeneratedType> auxiliaryGeneratedTypes,
            ...
            final JavaTypeName typeName, final ModuleGenerator module, final TypeEffectiveStatement<?> type,
            final boolean isTypedef) {
        final GeneratedTOBuilder builder = builderFactory.newGeneratedTOBuilder(typeName);
        builder.addImplementsType(BindingTypes.TYPE_OBJECT);
...
//        builder.setSchemaPath(typedef.getPath());
        ...
        for (EffectiveStatement<?, ?> stmt : type.effectiveSubstatements()) {
            if (stmt instanceof TypeEffectiveStatement) {
                ...
                final Type simpleType = SIMPLE_TYPES.get(subName);
                if (simpleType != null) {
                    final String propName = subName.getLocalName();
                    if (!builder.containsProperty(BindingMapping.getPropertyName(propName))) {
                        final GeneratedPropertyBuilder propBuilder = builder
                            .addProperty(propName)
                            .setReturnType(simpleType);
                ...
                // FIXME: further types, etc.

            }

//            if (type != null && !builder.containsProperty(propertyName)) {
//            }
...
//            (UnionTypeDefinition) baseTypedef, typedef);
        ...
        final GeneratedTOBuilder unionBuilder = builderFactory.newGeneratedTOBuilder(
            typeName.createSibling(typeName.simpleName() + "Builder"));
...
//  private static @NonNull Type resolveUnionSubtype(final TypeBuilderFactory builderFactory,
//          final EffectiveStatement<?, ?> definingStatement, final JavaTypeName typeName, final ModuleGenerator module,
//          final TypeEffectiveStatement<?> type, final boolean isTypedef) {
//        final QName arg = type.argument();
//        if (TypeDefinitions.BITS.equals(arg)) {
//            return createBits(builderFactory, typeName, module, type.getTypeDefinition(), isTypedef);
//        } else if (TypeDefinitions.ENUMERATION.equals(arg)) {
//            return createEnumeration(builderFactory, typeName, module, (EnumTypeDefinition) type.getTypeDefinition());
//        } else if (TypeDefinitions.UNION.equals(arg)) {
//            return createUnion(builderFactory, definingStatement, typeName, module, type, isTypedef);
//        } else {
//            return createSimple(builderFactory,
//                verifyNotNull(SIMPLE_TYPES.get(arg), "Unhandled type %s", arg));
//        }
//    }
