...
abstract class AbstractTypeObjectGenerator<T extends EffectiveStatement<?, ?>> extends AbstractDependentGenerator<T> {
    ...
    @NonNull ClassPlacement classPlacementImpl() {
        ...
        //       ease.
        return baseGen != null || SIMPLE_TYPES.containsKey(type.argument())
    ...
    final @NonNull Type methodReturnElementType(final @NonNull TypeBuilderFactory builderFactory) {
        ...
        final Restrictions restrictions = computeRestrictions();
        return restrictions == null || restrictions.isEmpty() ? baseType : Types.restrictedType(baseType, restrictions);
    }
    ...
    private @Nullable Restrictions computeRestrictions() {
        ...
        for (EffectiveStatement<?, ?> stmt : type.effectiveSubstatements()) {
            if (stmt instanceof TypeEffectiveStatement) {
                ...
                if (TypeDefinitions.UNION.equals(subName)) {
                    ...
                    if (baseType == null) {
                        ...
                        if (baseGen.refType instanceof TypeReference.Leafref) {
                            ...
                            if (builder.getProperties().stream()
                                .anyMatch(prop -> search == ((GeneratedPropertyBuilderImpl) prop).getReturnType())) {
                                continue;
                            }

                            // ... otherwise generate this weird property name
                            propSource = baseType.getName() + builder.getName() + "Value";
                        }
                    }
                ...
                if (builder.containsProperty(propName)) {
                     ...
                     *    }
                     *
                     *  We are ending up losing the information about 8..10 being an alternative. While we could merge
                     *  them into 'length 1..5|8..10', that makes the mapping complicated from both end user perspective
                     *  and implementation complexity.
                     *
                     *  We should instead use something simple -- like a counter of conflicting names, i.e. the second
                     *  string would be mapped to 'string1' or similar.
                     */
                    continue;
                }
