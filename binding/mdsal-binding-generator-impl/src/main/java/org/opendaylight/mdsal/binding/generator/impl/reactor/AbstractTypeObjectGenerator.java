...
abstract class AbstractTypeObjectGenerator<T extends EffectiveStatement<?, ?>> extends AbstractDependentGenerator<T> {
    ...
    final GeneratedType createTypeImpl(final TypeBuilderFactory builderFactory) {
        ...
        final QName typeName = type.argument();
        if (TypeDefinitions.BITS.equals(typeName)) {
            return createBits(builderFactory);
        } else if (TypeDefinitions.ENUMERATION.equals(typeName)) {
            return createEnumeration(builderFactory);
        } else if (TypeDefinitions.UNION.equals(typeName)) {
            return createUnion(builderFactory);
        } else {
            return createSimple(builderFactory,
                verifyNotNull(SIMPLE_TYPES.get(typeName), "Unhandled type %s", typeName));
        }
    }

    private @NonNull GeneratedTransferObject createBits(final TypeBuilderFactory builderFactory) {
    ...
    private @NonNull Enumeration createEnumeration(final TypeBuilderFactory builderFactory) {
        // enums are automatically Serializable
        final EnumTypeDefinition enumTypeDef = (EnumTypeDefinition) extractTypeDefinition();
    ...
    private @NonNull GeneratedTransferObject createUnion(final TypeBuilderFactory builderFactory) {
        final TypeDefinition<?> typedef = extractTypeDefinition();
...
//        method.setStatic(true);
    ...
    abstract @NonNull GeneratedType createSimple(@NonNull TypeBuilderFactory builderFactory, @NonNull Type javaType);
