...
//        builder.setSchemaPath(typedef.getPath());
        ...
        for (EffectiveStatement<?, ?> stmt : type.effectiveSubstatements()) {
            if (stmt instanceof TypeEffectiveStatement) {
                ...
                if (!builder.containsProperty(propName)) {
                    final Type generatedType = createUnionSubtype(auxiliaryGeneratedTypes, expressions, builder,
                        builderFactory, definingStatement, unionBaseTypes,
                        typeName.createEnclosed(BindingMapping.getClassName(localName), "$"), module, subType,
                        isTypedef);
    ...
    private static @NonNull Type createUnionSubtype(final List<GeneratedType> auxiliaryGeneratedTypes,
            final Map<String, String> expressions, final GeneratedTOBuilder unionBuilder,
            final TypeBuilderFactory builderFactory, final EffectiveStatement<?, ?> definingStatement,
            final Map<QName, TypedefGenerator> unionBaseTypes, final JavaTypeName typeName,
            final ModuleGenerator module, final TypeEffectiveStatement<?> type, final boolean isTypedef) {
        final QName arg = type.argument();
        if (TypeDefinitions.BITS.equals(arg)) {
            ...
            final GeneratedTransferObject ret = createUnion(auxiliaryGeneratedTypes, builderFactory, definingStatement,
                unionBaseTypes, typeName, module, type, isTypedef);
            unionBuilder.addEnclosingTransferObject(ret);
            return ret;
        }

        Type baseType = SIMPLE_TYPES.get(arg);
        if (baseType == null) {
...
//            }
            ...
            final TypedefGenerator baseGen = verifyNotNull(unionBaseTypes.get(arg), "Cannot resolve %s", arg);
