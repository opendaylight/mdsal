 ...
 */
abstract class AbstractTypeObjectGenerator<T extends EffectiveStatement<?, ?>, R extends RuntimeType>
        extends AbstractDependentGenerator<T, R> {
    ...
    final void linkDependencies(final GeneratorContext context) {
        ...
        final AbstractExplicitGenerator<T, R> prev = previous();
        if (prev != null) {
            verify(prev instanceof AbstractTypeObjectGenerator, "Unexpected previous %s", prev);
            ((AbstractTypeObjectGenerator<T, R>) prev).linkInferred(this);
        } else {
    ...
    final R createRuntimeType() {
        return createRuntimeType(methodReturnTypeElement != null ? methodReturnTypeElement
            : generatedType().orElseThrow());
    }
