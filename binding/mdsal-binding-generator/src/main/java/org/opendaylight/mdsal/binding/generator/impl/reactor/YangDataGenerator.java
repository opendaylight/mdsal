...
final class YangDataGenerator extends AbstractCompositeGenerator<YangDataEffectiveStatement, YangDataRuntimeType> {
    ...
    Member createMember(final CollisionDomain domain) {
        ...
        final var namingStrategy = identifier != null
            // We comply with YANG 'identifier' contract, piggy back to CamelCaseNamingStrategy and resolve conflicts
            // in our namespace
            ? new CamelCaseNamingStrategy(namespace(), identifier)
                // Generic string: this can be pretty much anything
                : new YangDataNamingStrategy(namespace(), templateName);

        // FIXME: CollisionDomain should be taking care of conflicts, so this should not be necessary...
        ...
        final boolean hasNameConflict = statement().effectiveSubstatements().stream()
                .anyMatch(stmt -> stmt.argument() instanceof AbstractQName qname
                    && templateName.equals(qname.getLocalName()));
