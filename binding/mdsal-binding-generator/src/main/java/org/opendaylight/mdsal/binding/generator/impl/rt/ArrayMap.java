 ...
 * </ul>
 */
abstract class ArrayMap<K, V> {
    private final Class<V[]> arrayType;
    private final Class<V> valueType;

    ArrayMap(final Class<V> valueType, final Class<V[]> arrayType) {
        this.valueType = requireNonNull(valueType);
        this.arrayType = requireNonNull(arrayType);
    }

    @NonNull Object index(final @NonNull Stream<? extends V> values) {
    ...
    final @NonNull Object indexFiltered(final Stream<? super GeneratedRuntimeType> values) {
        return index(values.filter(valueType::isInstance).map(valueType::cast));
    }
    ...
    private @Nullable V lookup(final V[] array, final @NonNull K key) {
        if (array.length == 0) {
            return null;
        }
        ...
        // around.
        final var offset = Arrays.binarySearch(array, null, (o1, o2) -> compareValue(requireNonNullElse(o1, o2), key));
        return offset < 0 ? null : array[offset];
    }

    final @Nullable V floor(final @NonNull Object values, final K key) {
        final var k = requireNonNull(key);
        if (arrayType.isInstance(values)) {
            return floor(arrayType.cast(values), k);
        }
        final var value = requireNonNull(valueType.cast(values));
        return compareValue(value, k) <= 0 ? value : null;
    }

    private @Nullable V floor(final V[] array, final @NonNull K key) {
        if (array.length == 0) {
            return null;
        }
        ...
        while (true) {
            final var highValue = array[high];
            if (compareValue(highValue, key) <= 0) {
                // Highest is smaller of equal: it is the answer
                return highValue;
            }
            if (high == low) {
                // Highest was the last value to check: not found
                return null;
            }

            // somewhere in [low .. high - 1]
            final var mid = (low + high) / 2;
            final var midValue = array[mid];
            final var cmp = compareValue(midValue, key);
            if (cmp == 0) {
                return midValue;
            }

            if (cmp < 0) {
                // mid is smaller, search [mid .. high - 1]
                high = high - 1;
                if (mid == high) {
                    return midValue;
                }
                ...
                // mid is greater, search [low .. mid - 1]
                if (mid == low) {
                    return null;
                }
                high = mid - 1;
    ...
    int size(final Object values) {
        if (arrayType.isInstance(values)) {
            return arrayType.cast(values).length;
        }
