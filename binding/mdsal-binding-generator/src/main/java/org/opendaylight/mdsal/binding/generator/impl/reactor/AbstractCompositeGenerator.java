...
public abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>>
    ...
    private final @NonNull Map<QName, SchemaTreeChild<?, ?>> schemaTreeChildren;
    private final List<Generator> childGenerators;
    ...
    @Override
    public Map<QName, SchemaTreeChild<?, ?>> schemaTreeChildren() {
    ...
    final void linkSchemaTreePlaceholders() {
        for (var child : schemaTreeChildren.values()) {
            if (child instanceof SchemaTreePlaceholder) {
                final var qname = child.getIdentifier();
                final var gen = verifyNotNull(findSchemaTreeGenerator(qname),
                    "Failed to find generator for child %s in %s", qname, this);
                ((SchemaTreePlaceholder<?, ?>) child).setGenerator(gen.getOriginal());
            }
        }
        for (var child : childGenerators) {
            if (child instanceof AbstractCompositeGenerator) {
                ((AbstractCompositeGenerator<?>) child).linkSchemaTreePlaceholders();
            }
        }
    }
    ...
    private Entry<List<Generator>, Map<QName, SchemaTreeChild<?, ?>>> createChildren(
        ...
        return Map.entry(List.copyOf(tmp), Maps.uniqueIndex(tmpSchema, SchemaTreeChild::getIdentifier));
    }
