...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends AbstractExplicitGenerator<T> {
    ...
    final @NonNull AbstractExplicitGenerator<?> getSchemaTreeGenerator(final SchemaNodeIdentifier path) {
        ...
        return getSchemaTreeGenerator(path.getNodeIdentifiers().iterator(), null);
    }

    private @NonNull AbstractExplicitGenerator<?> getSchemaTreeGenerator(final Iterator<QName> qnames,
        ...
        for (AbstractAugmentGenerator augment : augments) {
            final AbstractExplicitGenerator<?> gen = augment.findSchemaTreeGenerator(qname);
            if (gen != null) {
                return continueLookup(gen, qnames, null);
            }
        }
        ...
        for (GroupingGenerator grouping : groupings) {
            ...
            if (gen != null) {
                return continueLookup(gen, qnames, ns);
            }
        }
        ...
        return continueLookup(gen, qnames, localNamespace);
    }

    private static @NonNull AbstractExplicitGenerator<?> continueLookup(final @NonNull AbstractExplicitGenerator<?> gen,
        ...
        if (qnames.hasNext()) {
            ...
            return ((AbstractCompositeGenerator<?>) gen).getSchemaTreeGenerator(qnames, localNamespace);
        }
        return gen;
    }
