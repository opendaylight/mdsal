...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends AbstractExplicitGenerator<T> {
    ...
    long linkOriginalGenerator() {
        ...
        if (remaining == 0) {
            // FIXME: this bit needs to be split out into recursive part, which runs for a child after we have
            //        ascertained we do not have a child requirement there (otherwise its children would not find the
            //        augment).

//            private int continueAugmentLinkage() {
//                int ret = 0;
//
