...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends AbstractExplicitGenerator<T> {
     ...
     */
    private List<OriginalRequirement> requiredChildren = List.of();
    ...
    final void requireOriginalDescendant(final Iterator<QName> path,
        ...
        addRequirement(this instanceof GroupingEffectiveStatement
            ? new OriginalRequirement(callback, path, (GroupingGenerator) this)
                : new OriginalRequirement(callback, path));
    }

    void addRequirement(final OriginalRequirement req) {
        // FIXME: remove this verify
    ...
    final void addRequirement(final @NonNull Consumer<AbstractCompositeGenerator<?>> callback) {
        // FIXME: delay addition
        callback.accept(getOriginal());
    }
    ...
    final long linkOriginalGeneratorRecursive() {
        ...
        if (requiredChildren != null) {
            final var it = requiredChildren.iterator();
            while (it.hasNext()) {
                if (resolveChildRequirement(it.next())) {
                    progress++;
                    it.remove();
                }
            }
    ...
    private boolean resolveChildRequirement(final OriginalRequirement req) {
        ...
        final var adjusted = req.adjustedQName();
        final var gen = super.findSchemaTreeGenerator(adjusted);
        if (gen != null) {
            req.inLocal(gen);
            return true;
        }
