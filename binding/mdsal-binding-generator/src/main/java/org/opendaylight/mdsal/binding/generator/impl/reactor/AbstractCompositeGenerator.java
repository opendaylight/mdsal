...
abstract class AbstractCompositeGenerator<T extends EffectiveStatement<?, ?>> extends AbstractExplicitGenerator<T> {
    ...
    final void requireOriginalDescendant(final Iterator<QName> path, final @Nullable QNameModule localNamespace,
        ...
        addRequirement(new ChildRequirement(callback, path, path.next(), localNamespace));
    }

    private void addRequirement(final ChildRequirement req) {
        verifyNotNull(requiredChildren,
            "Attepted to add a requirement %s after all requirements have been resolved in %s", req.qname, this);
        if (requiredChildren.isEmpty()) {
            requiredChildren = new ArrayList<>(2);
        }
        requiredChildren.add(req);
    }
    ...
    final long linkOriginalGeneratorRecursive() {
        // FIXME: should check for empty and reserve null for not instantiated
        ...
        if (!unlinkedChildren.isEmpty()) {
            ...
            if (!nextUnlinked.isEmpty()) {
                ...
                // FIXME: we should not terminate here
                return progress;
            }
    ...
    private void resolveChildRequirement(final ChildRequirement req) {
        ...
        for (var augment : augments) {
            ...
            if (gen != null) {
                resolveChildRequirement(gen, req.callback, req.remaining, null);
                return;
            }
        }
        ...
        for (var grouping : groupings) {
            ...
            final var gen = grouping.findSchemaTreeGenerator(qname.bindTo(ns));
            if (gen != null) {
                resolveChildRequirement(gen, req.callback, req.remaining, ns);
                return;
            }
        }

        // Lastly try local statements adjusted with namespace, if applicable
        final var localNamespace = req.localNamespace;
        final var lookup = localNamespace == null ? qname : qname.bindTo(localNamespace);
        final var gen = verifyNotNull(super.findSchemaTreeGenerator(lookup),
            "Failed to find %s as %s in %s", qname, lookup, this);
        resolveChildRequirement(gen, req.callback, req.remaining, localNamespace);
    }

    private static void resolveChildRequirement(final AbstractExplicitGenerator<?> gen,
            final Consumer<AbstractCompositeGenerator<?>> callback, final Iterator<QName> remaining,
            final QNameModule localNamespace) {
        verify(gen instanceof AbstractCompositeGenerator, "Unexpected generator %s", gen);
        final var composite = (AbstractCompositeGenerator<?>) gen;

        if (remaining.hasNext()) {
            composite.addRequirement(new ChildRequirement(callback, remaining, remaining.next(), localNamespace));
        } else {
            callback.accept(composite.getOriginal());
        }
    }
