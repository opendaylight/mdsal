...
final class ListKeyTemplate extends ClassTemplate {
    ...
    override protected String formatDataForJavaDoc(GeneratedType type) {
        return '''
            This class represents the key of {@link «getListName(type)»} class.

            @see «getListName(type)»
        '''
    }

    private def String getListName(GeneratedType type) {
        if (!type.getImplements().isEmpty() && type.getImplements().get(0) instanceof ParameterizedType) {
            val implementsTypes = type.getImplements().get(0) as ParameterizedType
            if (implementsTypes.getActualTypeArguments().length > 0) {
                return implementsTypes.getActualTypeArguments().get(0).getName();
            }
        }
        return "";
    }
