...
final class JavaImportTracker {
    ...
    static JavaImportTracker forBuilderOf(final GeneratedType type) {
        // Since a builder would override methods of its interface, so it's appropriate
        // to inherit types of the interface (including nested classes which is the
        // return type of a getter).
        return forType(type);
    }
    ...
    String referenceString(final JavaTypeName typeName) {
        ...
        // We have to use FQCN, but we can not cache it by 'importedTypes'
        // for reuse (may try another structure) as it has conflicted with
        // local types or imported types which was checked in method 'importType',
        // otherwise it would lead to class name collisions in the compilation unit.
