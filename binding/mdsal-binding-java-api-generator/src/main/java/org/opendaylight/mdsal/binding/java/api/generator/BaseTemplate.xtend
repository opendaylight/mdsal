    ...
    def private static void appendSnippet(StringBuilder sb, GeneratedType type) {
        ...
        if (optDef.present) {
            ...
            if (def.nodes.empty) {
               ...
               .append("<pre>\n")
               .append(encodeAngleBrackets(encodeJavadocSymbols(YangTemplate.generateYangSnippet(def.module))))
               .append("</pre>")
            } else if (def.nodes.size == 1) {
                val schemaNode = def.nodes.iterator.next
                sb.append("<p>\n")
                .append("This class represents the following YANG schema fragment defined in module <b>")
                .append(def.module.name)
                .append("</b>\n")
                .append("<pre>\n")
                .append(encodeAngleBrackets(encodeJavadocSymbols(YangTemplate.generateYangSnippet(schemaNode))))
                .append("</pre>\n")
                .append("The schema path to identify an instance is\n")
                .append("<i>")
                .append(formatSchemaPath(def.module.name, schemaNode.path.pathFromRoot))
                .append("</i>\n")
                ...
                if (hasBuilderClass(schemaNode)) {
                    ...
                    if (schemaNode instanceof ListSchemaNode) {
                        val keyDef = schemaNode.keyDefinition
                ...
                sb.append("<pre>\n")
                for (SchemaNode schemaNode : def.nodes) {
