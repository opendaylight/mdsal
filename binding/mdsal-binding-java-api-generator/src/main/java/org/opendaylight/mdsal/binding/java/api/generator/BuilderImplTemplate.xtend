...
class BuilderImplTemplate extends AbstractBuilderTemplate {
    ...
    override getterMethod(GeneratedProperty field) '''
        ...
        «booleanCompatibilityGetter(field)»
    '''

    private def booleanCompatibilityGetter(GeneratedProperty field) {
        if (Types.isBooleanType(field.returnType)) {
            val getter = findGetter(field.getterMethodName.toString)
            val defType = getter.definingType
            checkArgument(defType instanceof GeneratedType, "Wrong defining type: %s", defType)
            val definingType = defType as GeneratedType
            if (definingType.specifiedGetters.contains(getter)) {
                return '''
