...
class InterfaceTemplate extends BaseTemplate {
    ...
    def private generateNonnullMethod(MethodSignature method) '''
        «val ret = method.returnType»
        «formatDataForJavaDoc(method, "@return " + asCode(ret.fullyQualifiedName) + " " + asCode(propertyNameFromGetter(method)))»
        «method.annotations.generateAnnotations»
        default «type.importedName(NONNULL.importedName)» «method.name»() {
            return «CodeHelpers.importedName».nonnull(«getGetterMethodForNonnull(method.name)»());
        }
    '''
