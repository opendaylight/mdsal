...
class InterfaceTemplate extends BaseTemplate {
    ...
    def private generateBindingHashCode() {
        ...
        return '''
             ...
             */
            «IF augmentable»
            static <T$$ extends «type.name» & «AugmentationHolder.importedName»<«type.name»>> int bindingHashCode(final @«NONNULL.importedName» T$$ obj) {
            «ELSE»
            static int «BINDING_HASHCODE_NAME»(final @«NONNULL.importedName» «type.name» obj) {
            «ENDIF»
                final int prime = 31;
                int result = 1;
