...
class BuilderTemplate extends BaseTemplate {
    ...
    def private checkArgument(GeneratedProperty field, Type actualType) '''
        «IF !(actualType instanceof GeneratedType) && getRestrictions(actualType) != null»
            ...
            «FOR currentConstant : type.getConstantDefinitions»
                    ...
                    && field.fieldName.toString.toUpperCase().equals(currentConstant.getName.substring(PATTERN_CONSTANT_NAME.length))
                    && currentConstant.getValue instanceof List»
                        for (Patten p : patterns«field.fieldName.toString») {
    ...
    def private generateSetter(GeneratedProperty field, Type actualType, boolean isPType) '''
        ...
        «IF isPType»
            public «type.getName»Builder set«field.getName.toFirstUpper»(final «field.getReturnType.getFullyQualifiedName»<«actualType.getFullyQualifiedName»> values) {
                if (values != null) {
    ...
    def private generateSetters() '''
        «FOR field : properties SEPARATOR '\n'»
             «IF field.getReturnType instanceof List»
