...
import static extension org.opendaylight.yangtools.yang.binding.contract.Naming.getSvcRootPackageName
...
final class YangModuleInfoTemplate {
    ...
    new(Module module, EffectiveModelContext ctx, Function<ModuleLike, Optional<String>> moduleFilePathResolver) {
        ...
        packageName = module.QNameModule.svcRootPackageName;
                ...
                «FOR imp : m.imports»
                    ...
                    «IF rev.empty»
                        ...
                        «FOR module : ctx.modules»
                            ...
                            «ENDIF»
                        «ENDFOR»
                        set.add(«sorted.lastEntry().value.QNameModule.svcRootPackageName».«MODULE_INFO_CLASS_NAME».getInstance());
                    «ELSE»
                        set.add(«(ctx.findModule(name, rev).orElseThrow.QNameModule).svcRootPackageName».«MODULE_INFO_CLASS_NAME».getInstance());
                    «ENDIF»
                «ENDFOR»
            «ENDIF»
