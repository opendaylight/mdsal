...
class ClassTemplate extends BaseTemplate {
    ...
    new(GeneratedTransferObject genType) {
        this(JavaImportTracker.forType(genType), genType)
    }
        ...
        «IF !type.enclosedTypes.empty»
            «FOR innerClass : type.enclosedTypes SEPARATOR "\n"»
                «IF (innerClass instanceof GeneratedTransferObject)»
                    «new ClassTemplate(importTracker, innerClass).generateAsInnerClass»
                «ENDIF»
            «ENDFOR»
        «ENDIF»
    '''
    ...
    def protected enumDeclarations() '''
        «IF !enums.empty»
            «FOR e : enums SEPARATOR "\n"»
                «new EnumTemplate(importTracker, e).generateAsInnerClass»
            «ENDFOR»
        «ENDIF»
    '''
