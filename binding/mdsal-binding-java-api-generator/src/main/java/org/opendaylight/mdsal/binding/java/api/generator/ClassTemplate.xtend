...
class ClassTemplate extends BaseTemplate {
    ...
    def protected constructors() '''
        «IF genTO.unionType»
            «genUnionConstructor»
        «ELSEIF genTO.typedef && allProperties.size == 1 && allProperties.get(0).name.equals(TypeUtils.VALUE_PROP)»
            «typedefConstructor»
        «ELSE»
    ...
    @«ConstructorParameters.importedName»("«TypeUtils.VALUE_PROP»")
    @«ConstructorProperties.importedName»("«TypeUtils.VALUE_PROP»")
    public «type.name»(«allProperties.asArgumentsDeclaration») {
        «IF false == parentProperties.empty»
            super(«parentProperties.asArguments»);
        «ENDIF»
