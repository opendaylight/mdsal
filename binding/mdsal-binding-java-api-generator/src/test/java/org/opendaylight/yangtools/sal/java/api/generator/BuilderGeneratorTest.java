...
public class BuilderGeneratorTest {
    ...
    private static final String APPEND_COMMA_AUGMENT = "\n    int builderLength = builder.length();\n"
            + "    if (builderLength > 2 && !builder.substring(builderLength - 2, builderLength).equals(\", \")) {\n"
            + "        " + APPEND_COMMA + "\n" + "    }";
    private static final String TEST = "test";
    private static final String AUGMENT_COMA_COMMENT =
            "//If there is more properties, then first property can contains data - this append comma behind\n"
                    + "    //value into builder - but the last property can't contain any data. If this happen, we have to\n"
                    + "    //check this situation and append comma into builder before augmentation only in case of builder\n"
                    + "    //doesn't contains comma at the end";
    ...
    public void builderTemplateGenerateToStringWithPropertyTest() throws Exception {
        ...
        assertTrue(generateToString.contains(GEN_TO_STRING_FIRST_PART
    ...
    public void builderTemplateGenerateToStringWithoutAnyPropertyTest() throws Exception {
        ...
        assertTrue(generateToString
                .contains(GEN_TO_STRING_FIRST_PART + GEN_TO_STRING_LAST_PART));
    }
    ...
    public void builderTemplateGenerateToStringWithoutPropertyWithAugmentTest() throws Exception {
        ...
        mockAugment(genType);
        final String generateToString = genToString(genType).toString();
        assertTrue(generateToString.contains(
                GEN_TO_STRING_FIRST_PART + GEN_TO_STRING_AUGMENT_PART + GEN_TO_STRING_LAST_PART));
    }
    ...
    public void builderTemplateGenerateToStringWithPropertyWithAugmentTest() throws Exception {
        ...
        assertTrue(generateToString.contains(GEN_TO_STRING_FIRST_PART
                + "\n    if (_test != null) {\n        builder.append(\"_test=\");\n        builder.append(_test);\n    }"
                + "\n    " + AUGMENT_COMA_COMMENT + APPEND_COMMA_AUGMENT + GEN_TO_STRING_AUGMENT_PART
                + GEN_TO_STRING_LAST_PART));
    }
