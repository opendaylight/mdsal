...
final class FileSearchUtil {
    ...
    static void assertFileContains(final File file, final String fileContent, final String ... linesToSearch) {
        for (final String line : linesToSearch) {
            assertFileContains(file, fileContent, line);
        }
        assertFileContains(file, fileContent, Joiner.on(LS).join(linesToSearch));
    }
