...
public final class ListsBindingUtils {
    ...
    public static DataObjectIdentifier.WithKey<TopLevelList, TopLevelListKey> path(final TopLevelListKey key) {
        return TOP_PATH.toBuilder().child(TopLevelList.class, key).build();
    }

    public static DataObjectIdentifier.WithKey<NestedList, NestedListKey> path(final TopLevelListKey top,
            final NestedListKey nested) {
        return path(top).toBuilder().child(NestedList.class, nested).build();
    }

    public static DataObjectIdentifier.WithKey<ListViaUses, ListViaUsesKey> path(final TopLevelListKey top,
