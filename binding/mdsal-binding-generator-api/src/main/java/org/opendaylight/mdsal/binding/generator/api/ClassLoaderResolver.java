...
public interface ClassLoaderResolver extends Immutable {
    public class LoadingStrategy implements ClassLoadingStrategy {
        private final ClassLoaderResolver resolver;

        public LoadingStrategy(ClassLoaderResolver resolver) {
            this.resolver = requireNonNull(resolver);
        }

        @Override
        public Class<?> loadClass(@Nullable String fullyQualifiedName) throws ClassNotFoundException {
     ...
     * @return ClassLoader if found.
     */
    default Optional<ClassLoader> findClassLoader(Type type) {
        return findClassLoader(requireNonNull(type.getFullyQualifiedName()));
    }

    default ClassLoadingStrategy asLoadingStrategy() {
