...
public class UnionTypeWithMultipleIdentityrefsTest extends AbstractBindingCodecTest {
    ...
    public void fromNNToBindingTest() {
        verifyIdentityWasTranslatedToBindingCorrectly(IDENTITY_ONE_QNAME, IdentOne.class);
        verifyIdentityWasTranslatedToBindingCorrectly(IDENTITY_TWO_QNAME, IdentTwo.class);
    }
    ...
    private void verifyIdentityWasTranslatedToBindingCorrectly(final QName identityQname,
        final Class<? extends BaseIdentity> identityClass) {
        final ContainerNode top = ImmutableContainerNodeBuilder
            .create(ImmutableNodes.containerNode(TOP_QNAME))
        ...
        Assert.assertNotNull(translated.getValue());
        Assert.assertTrue(translated.getValue() instanceof Top);
        final Top translatedTop = (Top) translated.getValue();
        assertEquals(identityClass.toString(), translatedTop.getTestUnionLeaf().stringValue());
    }
    ...
    private void verifyIdentityWasTranslatedToNNCorrectly(final UnionType chosenIdentity, final QName identityQname) {
        ...
        Assert.assertNotNull(translatedNN);
        ...
        Assert.assertTrue(leafNodeOpt.isPresent());
