...
public class CodecBySchemaNodeIdentifierTest extends AbstractBindingCodecTest {
    @Test
    ...
    @Test
    public void testRetrievingNotificationCodecContext() {
        final QName notificationQName = OutOfPixieDustNotification.QNAME;
        final String reasonValue = "test";

        final OutOfPixieDustNotification binding = new OutOfPixieDustNotificationBuilder()
                .setReason(reasonValue).build();

        final SchemaNodeIdentifier.Absolute pixieDust =
                SchemaNodeIdentifier.Absolute.of(notificationQName);

        final NotificationCodecContext<OutOfPixieDustNotification> notificationCodec =
                (NotificationCodecContext<OutOfPixieDustNotification>) codecContext.getSubtreeCodec(pixieDust);

        final YangInstanceIdentifier.NodeIdentifier reasonId =
                YangInstanceIdentifier.NodeIdentifier.create(QName.create(OutOfPixieDustNotification.QNAME,
                        "reason"));

        final ContainerNode dom = containerBuilder().withNodeIdentifier(YangInstanceIdentifier.NodeIdentifier
                .create(OutOfPixieDustNotification.QNAME)).withChild(leafBuilder().withNodeIdentifier(reasonId)
                .withValue(reasonValue).build()).build();

        assertEquals(notificationCodec.serialize(binding), dom);
    }
}
