...
public class BinaryKeyTest extends AbstractBindingCodecTest {
    ...
    private BinaryList process(final BinaryList binaryList) {
        final Entry<YangInstanceIdentifier, NormalizedNode<?, ?>> entry = codecContext.toNormalizedNode(instanceIdentifier,
            binaryList);
        return (BinaryList) codecContext.fromNormalizedNode(entry.getKey(), entry.getValue());
    }
}
