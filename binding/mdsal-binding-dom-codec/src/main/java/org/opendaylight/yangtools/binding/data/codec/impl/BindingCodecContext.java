...
final class BindingCodecContext implements CodecContextFactory, BindingCodecTree, Immutable {
    ...
    private static String getGetterName(final QName name, final TypeDefinition<?> typeDef) {
        final String suffix = BindingMapping.getGetterSuffix(name);
        ...
        for (final Method method : parentClass.getMethods()) {
            if (method.getParameterTypes().length == 0) {
                ...
                if (schema instanceof LeafSchemaNode) {
                    ...
                    continue; // We do not have schema for leaf, 
                              // so we will ignore it (eg. getClass, getImplementedInterface).
                }
