...
abstract class DataNodeContainerSerializerSource extends DataObjectSerializerSource {
    ...
    DataNodeContainerSerializerSource(final AbstractGenerator generator,
    ...
    protected CharSequence getSerializerBody() {
        ...
        build.append("{\n");
        build.append(statement(assign(DataObjectSerializerRegistry.class.getName(), REGISTRY, "$1")));
        build.append(statement(assign(dtoType.getFullyQualifiedName(), INPUT,
                cast(dtoType.getFullyQualifiedName(), "$2"))));
        build.append(statement(assign(BindingStreamEventWriter.class.getName(), STREAM,
                cast(BindingStreamEventWriter.class.getName(), "$3"))));
        build.append(statement(assign(BindingSerializer.class.getName(), SERIALIZER, null)));
        build.append("if (");
        build.append(STREAM);
        build.append(" instanceof ");
        build.append(BindingSerializer.class.getName());
        build.append(") {");
        build.append(statement(assign(SERIALIZER, cast(BindingSerializer.class.getName(), STREAM))));
        build.append('}');
        build.append(statement(emitStartEvent()));

        emitBody(build);
        emitAfterBody(build);
        build.append(statement(endNode()));
        build.append(statement("return null"));
        build.append('}');
        return build;
    }
     ...
     */
    protected void emitAfterBody(final StringBuilder build) {
        // No-op
    }
    ...
    private void emitBody(final StringBuilder build) {
        ...
        for (final DataSchemaNode schemaChild : schemaNode.getChildNodes()) {
            if (!schemaChild.isAugmenting()) {
                ...
                emitChild(build, getter, childType, schemaChild);
            }
        }
    }

    private void emitChild(final StringBuilder build, final String getterName, final Type childType,
            final DataSchemaNode schemaChild) {
        build.append(statement(assign(childType, getterName, cast(childType, invoke(INPUT, getterName)))));

        build.append("if (").append(getterName).append(" != null) {\n");
        emitChildInner(b, getterName, childType, schemaChild);
        build.append("}\n");
    }

    private void emitChildInner(final StringBuilder build, final String getterName, final Type childType,
            final DataSchemaNode child) {
        if (child instanceof LeafSchemaNode) {
            build.append(statement(leafNode(child.getQName().getLocalName(), getterName)));
        } else if (child instanceof AnyXmlSchemaNode) {
            build.append(statement(anyxmlNode(child.getQName().getLocalName(), getterName)));
            ...
            build.append(statement(startEvent));
            final Type valueType = ((ParameterizedType) childType).getActualTypeArguments()[0];
            build.append(forEach(getterName, valueType, statement(leafSetEntryNode(CURRENT))));
            build.append(statement(endNode()));
        } else if (child instanceof ListSchemaNode) {
            final Type valueType = ((ParameterizedType) childType).getActualTypeArguments()[0];
            final ListSchemaNode casted = (ListSchemaNode) child;
            emitList(build, getterName, valueType, casted);
        } else if (child instanceof ContainerSchemaNode) {
            build.append(tryToUseCacheElse(getterName,statement(staticInvokeEmitter(childType, getterName))));
            ...
            build.append(tryToUseCacheElse(getterName,statement(invoke(propertyName,
                    StreamWriterGenerator.SERIALIZE_METHOD_NAME, REGISTRY,
                        cast(DataObject.class.getName(),getterName), STREAM))));
        }
    }
    ...
    private StringBuilder tryToUseCacheElse(final String getterName, final CharSequence statement) {
        ...
        build.append("if ( ");
        build.append(SERIALIZER).append("== null || ");
        build.append(invoke(SERIALIZER, "serialize", getterName)).append("== null");
        build.append(") {");
    ...
    private void emitList(final StringBuilder build, final String getterName, final Type valueType,
            final ListSchemaNode child) {
        final CharSequence startEvent;

        build.append(statement(assign("int", "_count", invoke(getterName, "size"))));
        ...
        build.append(statement(startEvent));
        build.append(forEach(getterName, valueType, tryToUseCacheElse(CURRENT,
