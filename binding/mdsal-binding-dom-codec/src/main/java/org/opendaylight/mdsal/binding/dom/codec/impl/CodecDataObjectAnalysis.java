...
final class CodecDataObjectAnalysis<R extends CompositeRuntimeType> {
    ...
    private CodecDataObjectAnalysis(final DataContainerCodecPrototype<R> prototype, final CodecItemFactory itemFactory,
        ...
        final var leafChildBuilder =
            ImmutableMap.<String, ValueNodeCodecContext>builderWithExpectedSize(leafContexts.size());
        for (var leaf : leafContexts.values()) {
            leafChildBuilder.put(leaf.getSchema().getQName().getLocalName(), leaf);
            byYangBuilder.put(leaf.getDomPathArgument(), leaf);
        }

        leafNodes = leafChildBuilder.build();
