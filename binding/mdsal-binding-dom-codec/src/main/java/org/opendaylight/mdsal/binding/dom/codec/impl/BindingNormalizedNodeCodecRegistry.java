...
public class BindingNormalizedNodeCodecRegistry implements DataObjectSerializerRegistry,
    ...
    public ContainerNode toNormalizedNodeNotification(final Notification data) {
        return serializeDataObject((DataObject) data,
            (iface, writer) -> newNotificationWriter((Class<? extends Notification>) iface, writer));
    }

    @Override
    public ContainerNode toNormalizedNodeRpcData(final DataContainer data) {
        return serializeDataObject((DataObject) data,
            (iface, writer) -> newRpcWriter((Class<? extends DataContainer>) iface, writer));
    }

    private ContainerNode serializeDataObject(final DataObject data,
            final BiFunction<Class<?>, NormalizedNodeStreamWriter, BindingStreamEventWriter> newWriter) {
        final NormalizedNodeResult result = new NormalizedNodeResult();
        // We create DOM stream writer which produces normalized nodes
        final NormalizedNodeStreamWriter domWriter = ImmutableNormalizedNodeStreamWriter.from(result);
        @SuppressWarnings({ "unchecked", "rawtypes" })
        final Class<? extends DataObject> type = (Class) data.getImplementedInterface();
        final BindingStreamEventWriter writer = newWriter.apply(type, domWriter);
        try {
