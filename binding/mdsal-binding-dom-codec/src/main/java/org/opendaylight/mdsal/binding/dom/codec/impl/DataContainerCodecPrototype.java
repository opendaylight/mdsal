...
import org.opendaylight.mdsal.binding.runtime.api.AugmentationRuntimeType;
...
final class DataContainerCodecPrototype<T extends CompositeRuntimeType<?>> implements NodeContextSupplier {
    ...
    static <T extends RuntimeType<?>> DataContainerCodecPrototype<T> from(final Class<?> cls, final T schema,
            final CodecContextFactory factory) {
        return new DataContainerCodecPrototype<>(cls, NodeIdentifier.create(schema.getQName()), schema, factory);
    }

    static <T extends RuntimeType<?>> DataContainerCodecPrototype<T> from(final Item<?> bindingArg, final T schema,
            final CodecContextFactory factory) {
    ...
    private @NonNull DataContainerCodecContext<?, T> createInstance() {
        ...
        } else if (type instanceof AugmentationRuntimeType) {
