...
final class ChoiceNodeCodecContext<D extends DataObject> extends DataContainerCodecContext<D, ChoiceSchemaNode> {
    ...
    DataContainerCodecContext<?, ?> getCase(final PathArgument arg) {
        ...
        if (caseType.isPresent()) {
            ...
            return streamChild(caseType.get()).streamChild(type);
        }
        ...
        if (result == null) {
            ...
            if (!inexact.isEmpty()) {
                ...
                if (ambiguousByCaseChildWarnings.add(type)) {
                    LOG.warn("Ambiguous reference %s to child of %s resolved to first case in %s. This mapping is not "
                            + "guaranteed to be stable and is subject to variations based on runtime circumstances. "
                            + "Please see the stack trace for hints about the source of ambiguity.", type, bindingArg(),
                            result.getBindingClass(), inexact, new Throwable());
                }
            }
        }
