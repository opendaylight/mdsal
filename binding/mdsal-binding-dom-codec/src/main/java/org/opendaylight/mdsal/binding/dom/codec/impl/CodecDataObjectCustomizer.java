...
final class CodecDataObjectCustomizer implements ByteBuddyCustomizer {
    /**
     * Utility wrapper to force ASM to compute frames. This is needed for codecEquals() method, which contains jumps.
     */
    ...
    /**
     * IFEQ opcode invocation, jumping to a particular label. Used in codecEquals() implementation.
     */
    ...
    /**
     * A label definition, marking the spot where IfEq should jump. Used in codecEquals() implementation.
     */
    ...
    public <T> ByteBuddyResult<T> customize(final CodecClassLoader loader, final Class<?> bindingInterface,
        ...
        Builder<T> tmp = builder.visit(EnableFramesComputing.INSTANCE).implement(bindingInterface);
