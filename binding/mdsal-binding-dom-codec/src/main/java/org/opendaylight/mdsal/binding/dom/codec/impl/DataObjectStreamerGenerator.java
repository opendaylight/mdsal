...
final class DataObjectStreamerGenerator implements ByteBuddyCustomizer {
    ...
    static Class<? extends DataObjectStreamer> generateStreamer(final CodecClassLoader loader,
        ...
        final DataObjectStreamerGenerator generator = new DataObjectStreamerGenerator(registry, typeAndSchema.getKey(),
            (DataNodeContainer) schema, type, startEvent);

        return loader.generateSubclass(DataObjectStreamer.class, type, "streamer", generator);
    }
    ...
    public <T> ByteBuddyResult<T> customize(final CodecClassLoader loader, final Class<?> bindingInterface,
        ...
        final ByteBuddyResult<T> result = ByteBuddyResult.of(builder.defineMethod("serialize", BB_VOID, PUB_FINAL)
            .withParameters(BB_DOSR, BB_DATAOBJECT, BB_BESV).throwing(BB_IOX).intercept(
                new SerializeImplementation(bindingInterface, startEvent, children)).make(), depBuilder.build());
    ...
    private ChildStream createStream(final CodecClassLoader loader, final ImmutableMap<String, Type> props,
            final DataSchemaNode schema, final Method getter) {
        if (schema instanceof LeafSchemaNode) {
            return qnameChildStream(STREAM_LEAF, getter, schema);
        }
        ...
        if (schema instanceof ListSchemaNode) {
            ...
            return listChildStream(getter, valueClass.asSubclass(DataObject.class), (ListSchemaNode) schema);
        }
        if (schema instanceof ChoiceSchemaNode) {
            return choiceChildStream(getter);
        }
        if (schema instanceof AnyXmlSchemaNode) {
            return qnameChildStream(STREAM_ANYXML, getter, schema);
        }
        if (schema instanceof LeafListSchemaNode) {
            return qnameChildStream(((LeafListSchemaNode) schema).isUserOrdered() ? STREAM_ORDERED_LEAF_LIST
                    : STREAM_LEAF_LIST, getter, schema);
        }
