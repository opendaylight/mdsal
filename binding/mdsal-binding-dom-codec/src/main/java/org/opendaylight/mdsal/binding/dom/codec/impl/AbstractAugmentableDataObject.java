...
public abstract class AbstractAugmentableDataObject<T extends DataObject & Augmentable<T>>
        extends AbstractDataObject<T> implements Augmentable<T>, AugmentationHolder<T> {
    @SuppressWarnings("rawtypes")
    private static final AtomicReferenceFieldUpdater<AbstractAugmentableDataObject, ImmutableClassToInstanceMap>
            CACHED_AUGMENTATIONS_UPDATER = AtomicReferenceFieldUpdater.newUpdater(AbstractAugmentableDataObject.class,
    ...
    @Override
    public final <E$$ extends Augmentation<T>> @Nullable E$$ augmentation(final Class<E$$> augmentationType) {
        ...
        if (optAugCtx.isPresent()) {
            ...
            if (augmentationType.isAssignableFrom(augCtx.getBindingClass())) {
                ...
                if (augData.isPresent()) {
                    return (E$$) augCtx.deserialize(augData.get());
                }
            }
        }
        return null;
    ...
    public final Map<Class<? extends Augmentation<T>>, Augmentation<T>> augmentations() {
        ...
        local = ImmutableClassToInstanceMap.copyOf(context.getAllAugmentationsFrom(data));
