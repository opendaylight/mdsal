    ...
    private DataObjectCodecContext(final CommonDataObjectCodecPrototype<T> prototype,
        ...
        if (!augmentToPrototype.isEmpty()) {
            ...
            for (var entry : augmentToPrototype.entrySet()) {
                final var substites = context.getTypes().getSubstitutionsForAugment(entry.getValue().runtimeType());
                for (var sub : substites) {
                    substitutions.put(sub.getClass(), entry.getValue());
                }
            }
        }
