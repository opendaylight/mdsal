...
public abstract class DataObjectCodecContext<D extends DataObject, T extends CompositeRuntimeType>
    ...
    private @Nullable DataContainerCodecPrototype<?> loadMismatchedAugmentation(
        ...
        if (getBindingClass().equals(augTarget) && belongsToRuntimeContext(childClass)) {
            // grab context
            var context = augmentationByStream.values().size() == 0 ? null :
            ...
            AugmentRuntimeType childType = context.getAugmentationDefinition((Class)childClass);

            // get the precalculated possible substitutions
            List<AugmentRuntimeType> substitutables = context.getTypes().getSubstitutionsForAugment(childType);
            for (final Class<?> augBindingClass : augmentationByStream.keySet()) {
                AugmentRuntimeType augType = context.getAugmentationDefinition((Class)augBindingClass);
                if (Augmentation.class.isAssignableFrom(augBindingClass)
                        && substitutables.contains(augType)) {
                    var augPrototype = augmentationByStream.get(augBindingClass);
