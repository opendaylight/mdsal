...
final class SchemaRootCodecContext<D extends DataObject> extends DataContainerCodecContext<D,SchemaContext> {
    ...
    ActionCodecContext createActionContext(final Class<? extends Action<?, ?, ?>> action) {
        final Class<?> actionType = getSpecificActionInterface(action);
        if (actionType.equals(Action.class)) {
            return prepareActionContext(1, 2, 3, action, actionType);
        } else if (actionType.equals(KeyedListAction.class)) {
            return prepareActionContext(2, 3, 4, action, actionType);
        }
    ...
    private ActionCodecContext prepareActionContext(final int inputOffset, final int outputOffset,
        ...
        final ParameterizedType paramType = ClassLoaderUtils.findParameterizedType(action, actionType);
        Preconditions.checkNotNull(paramType, "There does not exist any ParameterType in " + action.getName());
    ...
    private Class<?> getSpecificActionInterface(final Class<? extends Action<?, ?, ?>> action) {
        final Class<?>[] interfaces = action.getInterfaces();
        Preconditions.checkArgument(interfaces.length == 1
                && (interfaces[0].equals(Action.class) || interfaces[0].equals(KeyedListAction.class)),
                "Specific action interface can be only extended by Action or KeyedListAction");
        return interfaces[0];
    }
