...
abstract class DataNodeContainerSerializerSource extends DataObjectSerializerSource {
    ...
    protected CharSequence getSerializerBody() {
        final StringBuilder sb = new StringBuilder()
                .append("{\n")
                .append(statement(assign(DataObjectSerializerRegistry.class.getName(), REGISTRY, "$1")))
                .append(statement(assign(dtoType.getFullyQualifiedName(), INPUT,
                    cast(dtoType.getFullyQualifiedName(), "$2"))))
                .append(statement(assign(BindingStreamEventWriter.class.getName(), STREAM,
                    cast(BindingStreamEventWriter.class.getName(), "$3"))))
                .append(statement(assign(BindingSerializer.class.getName(), SERIALIZER, null)))
                .append("if (")
                .append(STREAM)
                .append(" instanceof ")
                .append(BindingSerializer.class.getName())
                .append(") {")
                .append(statement(assign(SERIALIZER, cast(BindingSerializer.class.getName(), STREAM))))
                .append('}')
