...
public final class DataObjectStreamerCustomizer implements Customizer {
    ...
    public List<Class<?>> customize(final CodecClassLoader loader, final CtClass bindingClass, final CtClass generated)
        ...
        LOG.info("Definining streamer {}", generated.getName());
    ...
    @NonNull DataObjectStreamer<?> resolve(final @NonNull String methodName) {
        final Class<? extends DataObject> type = verifyNotNull(constants.get(methodName), "Cannot resolve type of %s",
            methodName);
        return verifyNotNull(registry.getDataObjectSerializer(type), "Cannot find serializer for %s", type);
    }
