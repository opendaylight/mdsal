...
class LazyDataObject<D extends DataObject> implements InvocationHandler, AugmentationReader {
    ...
    private Object getAugmentationImpl(final Class<?> cls) {
        ...
        for (;;) {
            ...
            if (cached != null) {
                // We have a cache: consult it
                final Augmentation<?> ret = cached.get(cls);
                if (ret != null || cached instanceof ImmutableMap) {
                    // Found a hit or we have an authoritative no
                    return ret;
                }

                // This has to be a partial cache, force full cache instantiation and perform a lookup
                return computeCachedAugmentations(cached).get(cls);
            }
            ...
            // Attempt to install a partial cache. If we fail, we either had a concurrent single or full initialization
            // and hence we need to retry.
    ...
    private Map<Class<? extends Augmentation<?>>, Augmentation<?>> computeCachedAugmentations(final Map<?, ?> prev) {
        ...
        // We have raced in instantiating the cache, read its new contents. We need to deal with the possibility of
        // racing on each step in null -> partial -> full, hence we try this twice
        for (int i = 0; i < 2; ++i) {
            ...
            if (CACHED_AUGMENTATIONS_UPDATER.compareAndSet(this, verifyNotNull(reRead), local)) {
                return local;
            }
        }

        throw new IllegalStateException("Failed to settle cache after two tries, currently have "
                + cachedAugmentations);
    }
