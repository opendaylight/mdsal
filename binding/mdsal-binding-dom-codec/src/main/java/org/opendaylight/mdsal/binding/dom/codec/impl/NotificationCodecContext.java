...
final class NotificationCodecContext<D extends DataObject & Notification>
    ...
    private static final MethodType NOTIFICATION_TYPE = MethodType.methodType(Notification.class,
        ContainerNode.class, Instant.class);
    ...
    NotificationCodecContext(final Class<?> key, final NotificationDefinition schema,
        ...
        try {
            ctor = MethodHandles.publicLookup().unreflectConstructor(
                awareClass.getConstructor(DataObjectCodecContext.class, NormalizedNodeContainer.class, Instant.class));
        } catch (IllegalAccessException | NoSuchMethodException e) {
            throw new LinkageError("Failed to acquire constructor", e);
        }

        eventProxy = ctor.bindTo(this).asType(NOTIFICATION_TYPE);
    }
    ...
    Notification deserialize(final @NonNull ContainerNode data, final @NonNull Instant eventInstant) {
        try {
            return (Notification) eventProxy.invokeExact(data, eventInstant);
    ...
    private enum ConstructorImplementation implements Implementation {
        ...
        static {
            ...
            try {
                ctor = AugmentableCodecDataObject.class.getDeclaredConstructor(
                    DataObjectCodecContext.class, NormalizedNodeContainer.class);
            } catch (NoSuchMethodException e) {
                throw new ExceptionInInitializerError(e);
            }

            LOAD_CTOR_ARGS = MethodVariableAccess.allArgumentsOf(new MethodDescription.ForLoadedConstructor(ctor));
        }
