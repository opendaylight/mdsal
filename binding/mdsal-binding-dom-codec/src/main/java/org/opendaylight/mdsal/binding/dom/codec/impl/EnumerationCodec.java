...
final class EnumerationCodec extends ReflectionBasedCodec implements SchemaUnawareCodec {
    ...
    static Callable<EnumerationCodec> loader(final Class<?> returnType, final EnumTypeDefinition def) {
        ...
        return () -> {
            // We want to translate to the String instance used by the schema
            final Map<String, String> assignedNames =  def.getValues().stream().map(EnumPair::getName)
            ...
            final Map<String, Enum<?>> mapping = Maps.uniqueIndex(Arrays.asList(enumType.getEnumConstants()),
                value -> {
                    checkArgument(value instanceof Enumeration,
                        "Enumeration constant %s is not implementing Enumeration", value);
                    final String name = ((Enumeration) value).getName();
                    final String yang = assignedNames.get(name);
                    if (yang == null) {
            ...
            for (String name : assignedNames.keySet()) {
