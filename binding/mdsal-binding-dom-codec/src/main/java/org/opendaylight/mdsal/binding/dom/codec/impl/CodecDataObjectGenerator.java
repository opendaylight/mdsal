...
final class CodecDataObjectGenerator<T extends CodecDataObject<?>> implements ClassGenerator<T> {
    ...
    private CodecDataObjectGenerator(final TypeDescription superClass,
    ...
    static <D extends DataObject, T extends CodecDataObject<T>> Class<T> generate(final CodecClassLoader loader,
        ...
        return loader.generateClass(bindingInterface, "codecImpl",
            new CodecDataObjectGenerator<>(BB_CDO, properties, keyMethod));
    }
    ...
    static <D extends DataObject, T extends CodecDataObject<T>> Class<T> generateAugmentable(
            ...
            final Entry<Method, IdentifiableItemCodec> keyMethod) {
        return loader.generateClass(bindingInterface, "codecImpl",
            new CodecDataObjectGenerator<>(BB_ACDO, properties, keyMethod));
    }
    ...
    public GeneratorResult<T> generateClass(final CodecClassLoader loeader, final String fqcn,
        ...
        Builder<T> builder = (Builder<T>) new ByteBuddy()
            ...
            .visit(ByteBuddyUtils.computeFrames());

        for (Method method : properties.keySet()) {
            LOG.trace("Generating for method {}", method);
            final String methodName = method.getName();
            builder = builder.defineMethod(methodName, method.getReturnType(), PUB_FINAL)
    ...
    private abstract static class AbstractMethodImplementation implements Implementation {
        ...
        abstract class AbstractMethod implements ByteCodeAppender {
            @Override
            public final Size apply(final MethodVisitor methodVisitor, final Context implementationContext,
                ...
                StackManipulation.Size operandStackSize = new StackManipulation.Compound(
                    ...
                    THIS,
                    getField(implementationContext, arfuName()),
                    getField(implementationContext, contextName()),
                    codecMember(),
        ...
        private final class ArfuInitializer implements ByteCodeAppender {
            @Override
        ...
        // initializer for the context field
        final class ContextInitializer implements ByteCodeAppender {
            private final StackManipulation resolveMethod;

            ContextInitializer(final StackManipulation resolveMethod) {
            ...
            public Size apply(final MethodVisitor methodVisitor, final Context implementationContext,
                    final MethodDescription instrumentedMethod) {
                StackManipulation.Size operandStackSize = new StackManipulation.Compound(
                    // ... = CodecDataObjectBridge.<RESOLVE_METHOD>("getFoo");
                    methodNameText(),
                    resolveMethod,
    ...
    private static final class GetMethodImplementation extends AbstractMethodImplementation {
        ...
        public InstrumentedType prepare(final InstrumentedType instrumentedType) {
            return super.prepare(instrumentedType
                ...
                .withInitializer(new ContextInitializer(BRIDGE_RESOLVE)));
        }
    ...
    private static final class KeyMethodImplementation extends AbstractMethodImplementation {
        ...
        private static final StackManipulation BRIDGE_RESOLVE = invokeMethod(CodecDataObjectBridge.class, "resolveKey",
            String.class);
        private static final StackManipulation CODEC_MEMBER = invokeMethod(CodecDataObject.class, "codecMember",
            AtomicReferenceFieldUpdater.class, IdentifiableItemCodec.class);
    ...
    private abstract static class AbstractAllPropertiesAppender implements ByteCodeAppender {
