...
abstract class CodecDataObjectGenerator<T extends CodecDataObject<?>> implements ClassGenerator<T> {
    ...
    private static final class Reusable<T extends CodecDataObject<?>> extends CodecDataObjectGenerator<T>
        ...
        Builder<T> generateGetters(final Builder<T> builder) {
            ...
            for (Entry<Method, Class<?>> entry : daoProperties.entrySet()) {
                ...
                final String nonnullName = BindingMapping.NONNULL_PREFIX
                    + methodName.substring(BindingMapping.GETTER_PREFIX.length());
                tmp = tmp.method(named(nonnullName).and(not(isDefaultMethod()))).intercept(
                        new NonnullMethodImplementation(nonnullName, retType, entry.getValue(), methodName));
            }
    ...
    private static final class NonnullMethodImplementation extends AbstractMethodImplementation {
        ...
        NonnullMethodImplementation(final String methodName, final TypeDescription retType,
                final Class<?> bindingClass, final String getterName) {
            super(methodName, retType);
            ...
            verify(getterName.startsWith(BindingMapping.GETTER_PREFIX), "Invalid method name %s", methodName);
