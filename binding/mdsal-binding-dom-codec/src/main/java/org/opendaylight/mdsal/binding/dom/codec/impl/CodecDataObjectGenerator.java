...
final class CodecDataObjectGenerator<T extends CodecDataObject<?>> implements ClassGenerator<T> {
    ...
    private final ImmutableMap<Method, NodeContextSupplier> contextProperties = ImmutableMap.of();
    ...
    static <D extends DataObject, T extends CodecDataObject<T>> Class<T> generate(final CodecClassLoader loader,
        ...
        return loader.generateClass(bindingInterface, "codecImpl",
            new CodecDataObjectGenerator<>(CDO, simpleProperties, daoProperties, keyMethod));
    }
    ...
    static <D extends DataObject, T extends CodecDataObject<T>> Class<T> generateAugmentable(
            ...
            final Map<Method, Class<?>> daoProperties, final Method keyMethod) {
        return loader.generateClass(bindingInterface, "codecImpl",
            new CodecDataObjectGenerator<>(ACDO, simpleProperties, daoProperties, keyMethod));
    }
    ...
    public GeneratorResult<T> generateClass(final CodecClassLoader loeader, final String fqcn,
        ...
        final List<Method> properties = new ArrayList<>(simpleProperties.size() + daoProperties.size()
            + contextProperties.size());
        properties.addAll(simpleProperties.keySet());
        properties.addAll(daoProperties.keySet());
        properties.addAll(contextProperties.keySet());
    ...
    @NonNull NodeContextSupplier resolve(final @NonNull String methodName) {
        final Optional<Entry<Method, NodeContextSupplier>> found = contextProperties.entrySet().stream()
