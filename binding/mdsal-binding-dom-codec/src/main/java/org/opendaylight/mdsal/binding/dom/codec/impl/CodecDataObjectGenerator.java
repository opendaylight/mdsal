...
final class CodecDataObjectGenerator<T extends CodecDataObject<?>> implements ClassGenerator<T> {
    ...
    private static Implementation codecEquals(final ImmutableMap<StackManipulation, Method> properties) {
        ...
        final StackManipulation[] manipulations = new StackManipulation[properties.size() * 6 + 5];
        int offset = 0;
        for (Entry<StackManipulation, Method> entry : properties.entrySet()) {
            // if (!java.util.(Objects|Arrays).equals(getFoo(), other.getFoo())) {
            //     return false;
            // }
            manipulations[offset++] = THIS;
            manipulations[offset++] = entry.getKey();
            manipulations[offset++] = FIRST_ARG_REF;
            manipulations[offset++] = entry.getKey();
            manipulations[offset++] = entry.getValue().getReturnType().isArray() ? ARRAYS_EQUALS : OBJECTS_EQUALS;
            manipulations[offset++] = ifFalse;
        }

        // return true;
        manipulations[offset++] = IntegerConstant.ONE;
        manipulations[offset++] = MethodReturn.INTEGER;
        // L0: return false;
        manipulations[offset++] = ByteBuddyUtils.markLabel(falseLabel);
        manipulations[offset++] = IntegerConstant.ZERO;
        manipulations[offset++] = MethodReturn.INTEGER;

        return new Implementation.Simple(manipulations);
    }

    private static Implementation codecFillToString(final ImmutableMap<StackManipulation, Method> properties) {
        final StackManipulation[] manipulations = new StackManipulation[properties.size() * 4 + 2];
        int offset = 0;
        // push 'return helper' to stack...
        manipulations[offset++] = FIRST_ARG_REF;
        for (Entry<StackManipulation, Method> entry : properties.entrySet()) {
            // .add("getFoo", getFoo())
            manipulations[offset++] = new TextConstant(entry.getValue().getName());
            manipulations[offset++] = THIS;
            manipulations[offset++] = entry.getKey();
            manipulations[offset++] = HELPER_ADD;
        }
        // ... execute 'return helper'
        manipulations[offset++] = MethodReturn.REFERENCE;
