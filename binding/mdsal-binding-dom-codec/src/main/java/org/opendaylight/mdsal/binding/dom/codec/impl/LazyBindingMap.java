...
final class LazyBindingMap<K extends Identifier<V>, V extends DataObject & Identifiable<K>>
    ...
    LazyBindingMap(final Unordered<V> codec, final MapNode mapNode) {
    ...
    private @NonNull State<K, V> loadLookup() {
        final State<K, V> created = new LazyBindingMapLookupState<>(codec, mapNode);
        final Object witness;
        return (witness = STATE.compareAndExchangeRelease(this, null, created)) == null ? created
                : (State<K, V>) witness;
    }

    @SuppressWarnings("unchecked")
    private @NonNull State<K, V> loadIter() {
        final State<K, V> created = new LazyBindingMapIterState<>(codec, mapNode);
        final Object witness;
        return (witness = STATE.compareAndExchangeRelease(this, null, created)) == null ? created
                : (State<K, V>) witness;
    }
