...
public abstract class CodecDataObject<T extends DataObject> implements DataObject {
    ...
    protected final Object nonnullMember(final VarHandle handle, final Class<? extends DataObject> bindingClass) {
        ...
        if (member == null) {
            try {
                final var loader = CodecClassLoader.create();
                final var builder = loader.loadClass(bindingClass.getName() + "Builder");
                return MethodHandles.publicLookup()
                        .findStatic(builder, "empty", MethodType.methodType(bindingClass))
                        .invoke();
