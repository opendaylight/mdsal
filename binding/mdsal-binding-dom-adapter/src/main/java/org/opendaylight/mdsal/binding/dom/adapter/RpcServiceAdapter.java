...
class RpcServiceAdapter implements InvocationHandler {
    ...
    private abstract class RpcInvocationStrategy {
        ...
        RpcInvocationStrategy(final QName rpcName) {
            outputPath = Absolute.of(rpcName, YangConstants.operationOutputQName(rpcName.getModule()).intern())
                    .intern();
    ...
    private final class NonRoutedStrategy extends RpcInvocationStrategy {
        ...
        ContainerNode serialize(final DataObject input) {
            return LazySerializedContainerNode.create(inputName(), input, adapterContext.currentSerializer());
        }
    }
    ...
    private final class RoutedStrategy extends RpcInvocationStrategy {
        ...
        ContainerNode serialize(final DataObject input) {
            ...
            if (bindingII != null) {
                ...
                return LazySerializedContainerNode.withContextRef(inputName(), input, contextRef, serializer);
            }
            return LazySerializedContainerNode.create(inputName(), input, serializer);
        }
