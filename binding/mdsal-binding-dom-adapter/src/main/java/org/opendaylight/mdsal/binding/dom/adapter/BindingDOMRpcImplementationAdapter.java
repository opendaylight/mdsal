...
final class BindingDOMRpcImplementationAdapter implements DOMRpcImplementation {
    ...
    <T extends RpcService> BindingDOMRpcImplementationAdapter(final AdapterContext adapterContext,
            final Class<T> type, final Map<Absolute, Method> localNameToMethod, final T delegate) {
        try {
            this.invoker = SERVICE_INVOKERS.get(type, () -> {
                final Map<QName, Method> map = new HashMap<>();
                for (Entry<Absolute, Method> e : localNameToMethod.entrySet()) {
    ...
    public ListenableFuture<DOMRpcResult> invokeRpc(final DOMRpcIdentifier rpc, final NormalizedNode<?, ?> input) {
        final Absolute rpcPath = rpc.getType();
        final CurrentAdapterSerializer serializer = adapterContext.currentSerializer();
        final DataObject bindingInput = input != null ? deserialize(serializer, rpcPath, input) : null;
        final ListenableFuture<RpcResult<?>> bindingResult = invoke(rpcPath, bindingInput);
    ...
    private DataObject deserialize(final CurrentAdapterSerializer serializer, final Absolute rpcPath,
        ...
        // FIXME: do not use intermediate SchemaPath here
        final SchemaPath inputSchemaPath = rpcPath.asSchemaPath().createChild(inputQname);
        return serializer.fromNormalizedNodeRpcData(inputSchemaPath, (ContainerNode) input);
    }

    private ListenableFuture<RpcResult<?>> invoke(final Absolute rpcPath, final DataObject input) {
        final List<QName> nodeIds = rpcPath.getNodeIdentifiers();
        return invoker.invokeRpc(delegate, nodeIds.get(nodeIds.size() - 1), input);
    }
}
