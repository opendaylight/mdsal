...
final class LazyDataTreeModification<T extends DataObject> implements DataTreeModification<T> {
    ...
    static <T extends DataObject> @Nullable DataTreeModification<T> from(final CurrentAdapterSerializer serializer,
        ...
        var bindingPath = serializer.coerceInstanceIdentifier(domChange.getRootPath());
        if (augment != null) {
            bindingPath = bindingPath.augmentation((Class) augment.asSubclass(Augmentation.class));
        }
    ...
    static <T extends DataObject> @Nullable DataTreeModification<T> from(final CurrentAdapterSerializer serializer,
        ...
        var bindingPath = serializer.coerceInstanceIdentifier(domRootPath.getRootIdentifier());
        if (augment != null) {
            bindingPath = bindingPath.augmentation((Class) augment.asSubclass(Augmentation.class));
        }
