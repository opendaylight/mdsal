...
final class BindingChangeListenerHelper {
    ...
    static <T extends DataObject, L extends DataTreeChangeListener<T>> L wrapListener(
        ...
        if (Augmentation.class.isAssignableFrom(path.getTargetType())) {
            ...
            return delegate instanceof ClusteredDataTreeChangeListener
                ? (L) new ClusteredAugmentationListenerWrapper<>(treeId, delegate, codec)
                : (L) new AugmentationListenerWrapper<>(treeId, delegate, codec);
        }
        return delegate;
    }

    private static class AugmentationListenerWrapper<T extends Augmentation<?>> implements DataTreeChangeListener<T> {
        ...
        private AugmentationListenerWrapper(final DataTreeIdentifier<T> treeId,
                final DataTreeChangeListener<T> delegate, final BindingAugmentationCodecTreeNode<T> codec) {
            augmentationType = treeId.getRootIdentifier().getTargetType();
            childPathArgs = codec.childPathArguments();
            childTypes = codec.childBindingClasses();
    ...
    private static final class ClusteredAugmentationListenerWrapper<T extends Augmentation<?>>
    ...
    private static final class AugmentationObjectNotification<T extends DataObject>
        ...
        @Override
        public <N extends Identifiable<K> & ChildOf<? super T>,
        ...
        public <H extends ChoiceIn<? super T> & DataObject, C extends Identifiable<K> & ChildOf<? super H>,
                K extends Identifier<C>> DataObjectModification<C> getModifiedChildListItem(
                    final Class<H> caseType, final Class<C> listItem, final K listKey) {
        ...
        public DataObjectModification<? extends DataObject> getModifiedChild(
                final InstanceIdentifier.PathArgument childArgument) {
