...
final class DefaultMatchBuilderPath<O extends DataObject, T extends DataObject> implements MatchBuilderPath<O, T> {
    ...
    public @NonNull <C extends ChildOf<O>> ObjectMatchBuilder<O, C> specificChild(
        SpecificChildObjectReference<O, C> methodRef) {
        return defaultFor(methodRef);
    }
    ...
    @Override
    public LeafListComparableMatchBuilder<O, BigDecimal> leafList(final Decimal64LeafListReference<T> methodRef) {
        return comparableFor(methodRef);
    }
    ...
    private <C extends ChildOf<O>> ObjectMatchBuilder<O, C> defaultFor(final SpecificChildObjectReference<O, C> ref) {
