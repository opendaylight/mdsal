...
sealed class RpcInvocationStrategy {
    ...
    static @NonNull RpcInvocationStrategy of(final RpcServiceAdapter adapter, final Method method,
        ...
        return new ContentRouted(adapter, schema.argument(), contentContext.leaf(),
            ContextReferenceExtractor.from(BindingReflections.resolveRpcInputClass(method)
                .orElseThrow(() -> new IllegalArgumentException("RPC method " + method.getName() + " has no input"))));
    }
