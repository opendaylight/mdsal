...
abstract sealed class RpcInvocationStrategy {
    ...
    static @NonNull RpcInvocationStrategy of(final AbstractRpcAdapter adapter, final Method method,
        ...
        final var rpcType = schema.getQName();
        final var routingStrategy = RpcRoutingStrategy.from(schema);
        return routingStrategy.isContextBasedRouted() ? new Routed(adapter, rpcType, method, routingStrategy.getLeaf())
                : new NonRouted(adapter, rpcType);
    }
