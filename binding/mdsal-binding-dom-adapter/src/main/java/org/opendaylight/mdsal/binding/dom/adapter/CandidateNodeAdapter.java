...
abstract sealed class CandidateNodeAdapter<T extends DataObject, N extends CommonDataObjectCodecTreeNode<T>> {
    ...
    private static void populateList(final ImmutableList.Builder<DataObjectModification<?>> result,
            ...
            final Collection<DataTreeCandidateNode> domChildren) {
        ...
        for (var entry : augmentChildren.asMap().entrySet()) {
            ...
            final var augChildren = entry.getValue().stream()
                .filter(childMod -> childMod.modificationType() != UNMODIFIED)
                .collect(ImmutableList.toImmutableList());
            // Only return a modification if there is something left
            if (!augChildren.isEmpty()) {
                final var modification = new OfAugmentation<>(entry.getKey(), parent, augChildren).toModification();
