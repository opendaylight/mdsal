...
final class ActionAdapter extends AbstracBindingAdapter<DOMOperationService> implements InvocationHandler {
    private static final Constructor<Lookup> LOOKUP_CTOR = AccessController.doPrivileged(
        (PrivilegedAction<Constructor<Lookup>>) () -> {
            final Constructor<Lookup> ctor;
            try {
                ...
                if (args.length == 2) {
                    ...
                    getDelegate().invokeAction(schemaPath,
                        ...
                        LazySerializedContainerNode.create(inputName, (DataObject) args[1],
                            ...
                            DOMOperationCallback.of(result -> {
                                // FIXME: instantiate proper result
                                ret.set(result);
                            }, failure -> {
                                // FIXME: map exceptions
    ...
    private static Lookup createLookup(final Class<?> type) {
        try {
            return LOOKUP_CTOR.newInstance(type, MethodHandles.Lookup.PRIVATE);
