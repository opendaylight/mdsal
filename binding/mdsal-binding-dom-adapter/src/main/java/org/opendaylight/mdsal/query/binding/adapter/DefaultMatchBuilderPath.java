...
final class DefaultMatchBuilderPath<O extends DataObject, T extends DataObject> implements MatchBuilderPath<O, T> {
    ...
    private <F> org.opendaylight.mdsal.binding.api.query.ValueMatchBuilder<O, F> defaultFor(final LeafReference<T, F> ref) {
        return new DefaultValueMatchBuilder<>(builder, select, builder.bindMethod(target.build(), ref));
    }

    private <F extends Comparable<F>> org.opendaylight.mdsal.binding.api.query.ComparableMatchBuilder<O, F> comparableFor(
