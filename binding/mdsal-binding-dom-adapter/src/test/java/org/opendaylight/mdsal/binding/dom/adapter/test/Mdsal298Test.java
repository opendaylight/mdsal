...
public class Mdsal298Test extends AbstractDataBrokerTest {
    ...
    public void testDataTreeModificationAddressable() throws InterruptedException, ExecutionException {
        final DataTreeChangeListener<AddressableCont> listener = assertWrittenAddressable();
    ...
    public void testDataTreeModificationUnaddressable() throws InterruptedException, ExecutionException {
        final DataTreeChangeListener<UnaddressableCont> listener = assertWrittenUnaddressable();
    ...
    private DataTreeChangeListener<AddressableCont> assertWrittenAddressable() throws InterruptedException,
            ExecutionException {
        final DataTreeChangeListener<AddressableCont> listener = mock(DataTreeChangeListener.class);
        doNothing().when(listener).onDataTreeChanged(any(Collection.class));
        getDataBroker().registerDataTreeChangeListener(ADDRESSABLE_CONTAINER_TID, listener);

        final DOMDataTreeWriteTransaction domTx = getDomBroker().newWriteOnlyTransaction();
        domTx.put(CONFIGURATION, YangInstanceIdentifier.create(ADDRESSABLE_CONTAINER_NID),
            Builders.containerBuilder()
            .withNodeIdentifier(ADDRESSABLE_CONTAINER_NID)
            .build());
        domTx.submit().get();
        ...
        final DataTreeModification<AddressableCont> change = capture.iterator().next();
        assertEquals(ADDRESSABLE_CONTAINER_TID, change.getRootPath());
        final DataObjectModification<AddressableCont> changedContainer = change.getRootNode();
        assertEquals(ModificationType.WRITE, changedContainer.getModificationType());
        assertEquals(new Item<>(AddressableCont.class), changedContainer.getIdentifier());

        final AddressableCont containerAfter = changedContainer.getDataAfter();
        assertEquals(new AddressableContBuilder().build(), containerAfter);
    ...
    private DataTreeChangeListener<UnaddressableCont> assertWrittenUnaddressable() throws InterruptedException,
            ExecutionException {
        final DataTreeChangeListener<UnaddressableCont> listener = mock(DataTreeChangeListener.class);
        doNothing().when(listener).onDataTreeChanged(any(Collection.class));
        getDataBroker().registerDataTreeChangeListener(UNADDRESSABLE_CONTAINER_TID, listener);

        final DOMDataTreeWriteTransaction domTx = getDomBroker().newWriteOnlyTransaction();
        domTx.put(CONFIGURATION, YangInstanceIdentifier.create(UNADDRESSABLE_CONTAINER_NID),
            ImmutableNodes.containerNode(UnaddressableCont.QNAME));
        domTx.submit().get();
        ...
        final DataTreeModification<UnaddressableCont> change = capture.iterator().next();
        assertEquals(UNADDRESSABLE_CONTAINER_TID, change.getRootPath());
        final DataObjectModification<UnaddressableCont> changedContainer = change.getRootNode();
        assertEquals(ModificationType.WRITE, changedContainer.getModificationType());
        assertEquals(new Item<>(UnaddressableCont.class), changedContainer.getIdentifier());

        final UnaddressableCont containerAfter = changedContainer.getDataAfter();
        assertEquals(new UnaddressableContBuilder().build(), containerAfter);
