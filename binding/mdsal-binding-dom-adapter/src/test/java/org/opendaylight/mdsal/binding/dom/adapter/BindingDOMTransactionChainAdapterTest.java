...
public class BindingDOMTransactionChainAdapterTest {
    enum TransactionChainType implements BiFunction<DataBroker, TransactionChainListener, TransactionChain> {
        NORMAL {
            ...
            @Override
            void mockRead(final DOMTransactionChain transactionChain) {
                doReturn(mock(DOMDataTreeReadTransaction.class)).when(transactionChain).newReadOnlyTransaction();
            }

            @Override
            void mockWrite(final DOMTransactionChain transactionChain) {
        ...
        abstract void mockRead(DOMTransactionChain transactionChain);

        abstract void mockWrite(DOMTransactionChain transactionChain);

        static void mockReadWrite(final DOMTransactionChain transactionChain) {
            doReturn(mock(DOMDataTreeReadWriteTransaction.class)).when(transactionChain).newReadWriteTransaction();
        }
    }
    ...
    public void readWriteTransactionTest() throws Exception {
        type.mockReadWrite(transactionChain);
