...
public sealed interface Archetype extends Type {
    ...
    non-sealed interface Key extends Archetype, WithStatement {
        ...
        JavaTypeName mapName();

        List<Leaf> leafs();

        // FIXME: 'Type' or something else?
        record Leaf(LeafEffectiveStatement stmt, String fieldName, Type type) {
            public Leaf {
                requireNonNull(stmt);
    ...
    sealed interface TypeObject extends Archetype, WithStatement {
         ...
         */
        non-sealed interface Bits extends TypeObject {
            @Override
            JavaConstruct.Class construct();
            ...
            record Field(BitEffectiveStatement stmt, String javaName) {
                public Field {
                    requireNonNull(stmt);
         ...
         */
        non-sealed interface Union extends TypeObject {
            @Override
            JavaConstruct.Class construct();
            ...
            List<TypeObject> generatedTypes();
