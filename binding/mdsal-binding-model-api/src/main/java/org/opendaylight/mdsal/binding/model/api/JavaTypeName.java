...
public abstract sealed class JavaTypeName implements Comparable<JavaTypeName>, Identifier, Immutable {
    ...
    private static final class Nested extends Reference {
        ...
        @Override
        int compareLocalName(final JavaTypeName other) {
            final var otherEnclosing = other.immediatelyEnclosingClass();
            if (otherEnclosing == null) {
                return compareLocalName(other.simpleName);
            }
            final int cmp = immediatelyEnclosingClass.compareLocalName(otherEnclosing);
            return cmp != 0 ? cmp : super.compareLocalName(other.simpleName);
        }

        @Override
        int compareLocalName(final String otherSimpleName) {
            final int cmp = immediatelyEnclosingClass.compareLocalName(otherSimpleName);
            return cmp != 0 ? cmp : 1;
        }
    ...
    public final int compareTo(final JavaTypeName o) {
        ...
        final var cmp = packageName().compareTo(o.packageName());
        return cmp != 0 ? cmp : compareLocalName(o);
    }

    int compareLocalName(final JavaTypeName other) {
        final var otherEnclosing = other.immediatelyEnclosingClass();
        return otherEnclosing == null ? compareLocalName(other.simpleName) : -other.compareLocalName(simpleName);
    }

    int compareLocalName(final String otherSimpleName) {
        return simpleName.compareTo(otherSimpleName);
    }
