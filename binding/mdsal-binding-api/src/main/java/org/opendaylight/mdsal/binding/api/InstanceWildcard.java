...
public class InstanceWildcard<T extends DataObject>
    ...
    private <N extends DataObject & KeyAware<K>, K extends Key<N>>
            @NonNull KeyedInstanceWildcard<N, K> childIdentifier(final IdentifiableItem<N, K> arg) {
     ...
     */
    public <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>> @NonNull KeyedInstanceWildcard<N, K> child(
    ...
    public <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>, N extends KeyAware<K> & ChildOf<? super C>>
            @NonNull KeyedInstanceWildcard<N, K> child(final Class<@NonNull C> caze, final Class<@NonNull N> listItem,
                final K listKey) {
    ...
    public static <N extends KeyAware<K> & DataObject, K extends Key<N>> K keyOf(final InstanceWildcard<N> id) {
        requireNonNull(id);
    ...
    static <N extends DataObject> @NonNull InstanceWildcard<N> trustedCreate(final PathArgument arg,
        ...
        for (var pa : pathArguments) {
            if (KeyAware.class.isAssignableFrom(pa.getType()) && pa instanceof Item<?>) {
                if (KeyAware.class.isAssignableFrom(arg.getType())) {
                    Key<?> key = null;
                    if (arg instanceof IdentifiableItem<?, ?> keyArg) {
                        key = keyArg.getKey();
                    }
    ...
    //there is no yang.binding.KeyedInstanceIdentifier that is wildcarded, hence no method in KeyedInstanceWildcard
    //for proof see yang.binding.InstanceIdentifier.trustedCrete() behavior
    @SuppressWarnings({"rawtypes", "unchecked"})
    public static <T extends DataObject> @NonNull InstanceWildcard<T> ofLegacy(
        ...
        Preconditions.checkArgument(legacyIdentifier.isWildcarded(), """
                Legacy InstanceIdentifier must be wildcarded. Use InstanceIdentifier.ofLegacy() instead.
                """);
        final List<PathArgument> newPathArguments = InstanceIdentifier.ofLegacyPathArguments(legacyIdentifier
                .getPathArguments());
        if (newPathArguments.get(newPathArguments.size() - 1) instanceof InstanceIdentifier.IdentifiableItem<?,?> ii) {
            return new KeyedInstanceWildcard(legacyIdentifier.getTargetType(), newPathArguments, legacyIdentifier
                    .hashCode(), ii.getKey());
        } else {
            return new InstanceWildcard<>(legacyIdentifier.getTargetType(), newPathArguments, legacyIdentifier
                    .hashCode());
        }
    }
    ...
    public org.opendaylight.yangtools.yang.binding.@NonNull InstanceIdentifier<T> toLegacy() {
        return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.unsafeOf(InstanceIdentifier
    ...
    public interface WildcardBuilder<T extends DataObject> {
         ...
         * @throws NullPointerException if {@code container} is null
         */
        <C extends ChoiceIn<? super T> & DataObject, N extends ChildOf<? super C>> @NonNull WildcardBuilder<N> child(
         ...
         * @throws NullPointerException if any argument is null
         */
        <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>> @NonNull KeyedWildcardBuilder<N, K> child(
        ...
        <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>, N extends KeyAware<K> & ChildOf<? super C>>
            @NonNull KeyedWildcardBuilder<N, K> child(Class<C> caze, Class<N> listItem, K listKey);
         ...
         * @throws NullPointerException if {@code container} is null
         */
        <N extends DataObject & Augmentation<? super T>> @NonNull WildcardBuilder<N> augmentation(Class<N> container);
