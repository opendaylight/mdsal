...
public interface ActionService extends BindingService {
    ...
    default <O extends DataObject, T extends Action<O, ?, ?>> T getActionHandle(Class<T> actionInterface) {
        return getActionHandle(actionInterface, ImmutableSet.of());
    }

    default <O extends DataObject, T extends Action<O, ?, ?>> T getActionHandle(Class<T> actionInterface,
            LogicalDatastoreType dataStore, InstanceIdentifier<O> path) {
    ...
    default <O extends DataObject, T extends Action<O, ?, ?>> T getActionHandle(Class<T> actionInterface,
            @SuppressWarnings("unchecked") DataTreeIdentifier<O>... nodes) {
