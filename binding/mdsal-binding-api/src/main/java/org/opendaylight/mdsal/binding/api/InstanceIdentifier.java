...
public class InstanceIdentifier<T extends DataObject>
    ...
    InstanceIdentifier(final Class<T> type, final Iterable<PathArgument> pathArguments, final int hash) {
    ...
    public final <N extends KeyAware<K> & DataObject, K extends Key<N>> @Nullable K firstKeyOf(
        ...
        for (var pathArg : pathArguments) {
            if (listItem.equals(pathArg.getType())) {
                @SuppressWarnings("unchecked")
                final K ret = ((IdentifiableItem<N, K>)pathArg).getKey();
                return ret;
            }
        }
    ...
    private <N extends DataObject> @NonNull InstanceIdentifier<N> childIdentifier(final Item<N> arg) {
        Preconditions.checkArgument(!Identifiable.class.isAssignableFrom(arg.getType()),
                "Child argument is Identifiable<?>."
                        + "Provide a key or use the respective method prefixed with \"wildcard\" instead.");
        return new InstanceIdentifier<>(arg.getType(), Iterables.concat(pathArguments, Collections.singleton(arg)),
                HashCodeBuilder.nextHashCode(hash, arg));
    }

    private <N extends DataObject & KeyAware<K>, K extends Key<N>>
            @NonNull KeyedInstanceIdentifier<N, K> childIdentifier(final IdentifiableItem<N, K> arg) {
        return new KeyedInstanceIdentifier<>(arg.getType(), Iterables.concat(pathArguments,
                Collections.singleton(arg)), HashCodeBuilder.nextHashCode(hash, arg), arg.getKey());
    }

    private <N extends DataObject & KeyAware<?>> @NonNull InstanceWildcard<N> wildcardChildIdentifier(
            final Item<N> arg) {
        return new InstanceWildcard<>(arg.getType(), Iterables.concat(pathArguments, List.of(arg)),
                HashCodeBuilder.nextHashCode(hash, arg));
    }
     ...
     */
    public <N extends ChildOf<? super T>> @NonNull InstanceIdentifier<N> child(final Class<@NonNull N> container) {
     ...
     */
    public <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>> @NonNull KeyedInstanceIdentifier<N, K> child(
    ...
    public <C extends ChoiceIn<? super T> & DataObject, N extends ChildOf<? super C>>
            @NonNull InstanceIdentifier<N> child(final Class<@NonNull C> caze,
    ...
    public <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>, N extends KeyAware<K> & ChildOf<? super C>>
            @NonNull KeyedInstanceIdentifier<N, K> child(final Class<@NonNull C> caze, final Class<@NonNull N> listItem,
                final K listKey) {
        return childIdentifier(IdentifiableItem.of(caze, listItem, listKey));
    }

    public <N extends ChildOf<? super T> & KeyAware<?>> @NonNull InstanceWildcard<N> wildcardChild(
            final Class<@NonNull N> container) {
        return wildcardChildIdentifier(Item.of(container));
    }

    public <C extends ChoiceIn<? super T> & DataObject, N extends ChildOf<? super C> & KeyAware<?>>
        @NonNull InstanceWildcard<N> wildcardChild(final Class<@NonNull C> caze,
            final Class<@NonNull N> container) {
        return wildcardChildIdentifier(Item.of(caze, container));
    }
     ...
     * @throws NullPointerException if {@code container} is null
     */
    public <N extends DataObject & Augmentation<? super T>> @NonNull InstanceIdentifier<N> augmentation(
            final Class<@NonNull N> container) {
        return childIdentifier(Item.of(container));
    }

    public <N extends DataObject & Augmentation<? super T> & KeyAware<?>>
            @NonNull InstanceWildcard<N> wildcardAugmentation(final Class<@NonNull N> container) {
        return wildcardChildIdentifier(Item.of(container));
    }
    ...
    public static <N extends KeyAware<K> & ChildOf<? extends DataRoot>, K extends Key<N>>
            @NonNull KeyedInstanceIdentifierBuilder<N, K> builder(final Class<N> listItem, final K listKey) {
    ...
    public static <C extends ChoiceIn<? extends DataRoot> & DataObject, N extends KeyAware<K> & ChildOf<? super C>,
            K extends Key<N>> @NonNull KeyedInstanceIdentifierBuilder<N, K> builder(final Class<C> caze,
                final Class<N> listItem, final K listKey) {
        return new InstanceIdentifierBuilderImpl<N>().addNode(IdentifiableItem.of(caze, listItem, listKey));
    }

    public static <T extends ChildOf<? extends DataRoot> & KeyAware<?>> @NonNull WildcardBuilder<T> wildcardBuilder(
            final Class<T> container) {
        return new InstanceIdentifierBuilderImpl<T>().addWildNode(Item.of(container));
    }

    public static <C extends ChoiceIn<? extends DataRoot> & DataObject, T extends ChildOf<? super C> & KeyAware<?>>
            @NonNull WildcardBuilder<T> wildcardBuilder(final Class<C> caze, final Class<T> container) {
    ...
    public static <R extends DataRoot & DataObject, C extends ChoiceIn<? super R> & DataObject,
        ...
        // FIXME: we are losing root identity, hence namespaces may not work correctly
        return new InstanceIdentifierBuilderImpl<N>().addNode(
                IdentifiableItem.of(caze, listItem, listKey));
    }

    public static <R extends DataRoot & DataObject, T extends ChildOf<? super R> & KeyAware<?>>
            @NonNull WildcardBuilder<T> wildcardBuilderOfInherited(final Class<R> root, final Class<T> container) {
        // FIXME: we are losing root identity, hence namespaces may not work correctly
        return new InstanceIdentifierBuilderImpl<T>().addWildNode(Item.of(container));
    }

    public static <R extends DataRoot & DataObject, C extends ChoiceIn<? super R> & DataObject,
            T extends ChildOf<? super C> & KeyAware<?>>
        @NonNull WildcardBuilder<T> wildcardBuilderOfInherited(final Class<R> root,
            final Class<C> caze, final Class<T> container) {
        // FIXME: we are losing root identity, hence namespaces may not work correctly
        return new InstanceIdentifierBuilderImpl<T>().addWildNode(Item.of(caze, container));
    }
    ...
    public static <N extends KeyAware<K> & DataObject, K extends Key<N>> K keyOf(final InstanceIdentifier<N> id) {
        requireNonNull(id);
        checkArgument(id instanceof KeyedInstanceIdentifier, "%s does not have a key", id);

        return ((KeyedInstanceIdentifier<N, K>)id).getKey();
    }
    ...
    static <N extends DataObject> @NonNull InstanceIdentifier<N> trustedCreate(final PathArgument arg,
        ...
        for (var pa : pathArguments) {
            final Class<?> type = pa.getType();
            checkArgument(!Identifiable.class.isAssignableFrom(type) || !(arg instanceof Item<?>),
                    "%s is not a valid path argument. Use InstanceWildcard.trustedCreate() instead", type);
        }
        if (KeyAware.class.isAssignableFrom(arg.getType())) {
            if (arg instanceof IdentifiableItem<?, ?> keyItem) {
                return new KeyedInstanceIdentifier(arg.getType(), pathArguments, hash, keyItem.getKey());
            }
            throw new IllegalStateException("Use InstanceWildcard.trustedCreate() instead.");
        }

        return new InstanceIdentifier(arg.getType(), pathArguments, hash);
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    public static <T extends DataObject> @NonNull InstanceIdentifier<T> ofLegacy(
            final org.opendaylight.yangtools.yang.binding.InstanceIdentifier<T> legacyIdentifier) {
        Preconditions.checkArgument(!legacyIdentifier.isWildcarded(), """
                Legacy InstanceIdentifier must not be wildcarded. Use InstanceWildcard.ofLegacy() instead.
                """);
        if (legacyIdentifier instanceof org.opendaylight.yangtools.yang.binding.KeyedInstanceIdentifier<?,?> kii) {
            return new KeyedInstanceIdentifier(legacyIdentifier.getTargetType(), ofLegacyPathArguments(kii
                    .getPathArguments()), legacyIdentifier.hashCode(), kii.getKey());
        }
        return new InstanceIdentifier<>(legacyIdentifier.getTargetType(), ofLegacyPathArguments(
                legacyIdentifier.getPathArguments()), legacyIdentifier.hashCode());
    }

    public org.opendaylight.yangtools.yang.binding.@NonNull InstanceIdentifier<T> toLegacy() {
        return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.unsafeOf(toLegacyPathArguments(
                pathArguments));
    }

    @SuppressWarnings("unchecked")
    static <C extends ChoiceIn<?> & DataObject, T extends ChildOf<? super C> & KeyAware<K>,
            K extends Key<T>> org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument
        toLegacyPathArgument(final PathArgument arg) {
        final Class<T> type = (Class<T>)arg.getType();
        if (arg instanceof Item<?> item) {
            if (item instanceof CaseItem<?, ?> caseItem) {
                final Class<C> caseType = (Class<C>) caseItem.getCaseType().orElseThrow(() ->
                        new IllegalArgumentException("Corrupted path argument"));
                return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.Item.of(caseType, type);
            }
            else {
                return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.Item.of(type);
            }
        } else if (arg instanceof IdentifiableItem<?, ?> identifiableItem) {
            final K key = (K)identifiableItem.getKey();
            if (identifiableItem instanceof CaseIdentifiableItem<?, ?, ?> caseItem) {
                final Class<C> caseType = (Class<C>) caseItem.getCaseType().orElseThrow(() ->
                        new IllegalArgumentException("Corrupted path argument"));
                return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.IdentifiableItem.of(caseType, type,
                        key);
            } else {
                return org.opendaylight.yangtools.yang.binding.InstanceIdentifier.IdentifiableItem.of(type, key);
            }
        }
    ...
    @SuppressWarnings("unchecked")
    static <C extends ChoiceIn<?> & DataObject, T extends ChildOf<? super C> & KeyAware<K>, K extends Key<T>>
            PathArgument ofLegacyPathArgument(
                final org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument arg) {
        final Class<T> type = (Class<T>)arg.getType();
        final Optional<Class<C>> caseTypeOpt = (Optional<Class<C>>)arg.getCaseType();
        if (caseTypeOpt.isEmpty()) {
            if (arg instanceof org.opendaylight.yangtools.yang.binding.InstanceIdentifier.Item<?>) {
                return Item.of(type);
            } else if (arg instanceof org.opendaylight.yangtools.yang.binding.InstanceIdentifier.IdentifiableItem<?,?>
                    identifiableItem) {
                return IdentifiableItem.of(type, (K)identifiableItem.getKey());
            }
            throw new IllegalArgumentException();
        } else {
            if (arg instanceof org.opendaylight.yangtools.yang.binding.InstanceIdentifier.Item<?>) {
                return Item.of(caseTypeOpt.orElseThrow(), type);
            } else if (arg instanceof org.opendaylight.yangtools.yang.binding.InstanceIdentifier.IdentifiableItem<?,?>
                identifiableItem) {
                return IdentifiableItem.of(caseTypeOpt.orElseThrow(), type, (K)identifiableItem.getKey());
            }
            throw new IllegalArgumentException();
        }
    }

    static List<PathArgument> ofLegacyPathArguments(final
            Iterable<org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument> legacyArgs) {
        final List<PathArgument> translatedPathArguments = new ArrayList<>();
        for (org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument pa : legacyArgs) {
            translatedPathArguments.add(ofLegacyPathArgument(pa));
        }
        return translatedPathArguments;
    }

    static List<org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument> toLegacyPathArguments(
            final Iterable<PathArgument> args) {
        final List<org.opendaylight.yangtools.yang.binding.InstanceIdentifier.PathArgument> legacyArgs = new
                ArrayList<>();
        for (PathArgument pa : args) {
            legacyArgs.add(toLegacyPathArgument(pa));
        }
        return legacyArgs;
    }
    ...
    abstract static class AbstractPathArgument<T extends DataObject> implements PathArgument, Serializable {
        @Serial
        ...
        public final boolean equals(final Object obj) {
            ...
            if (!(obj instanceof final AbstractPathArgument<?> other)) {
                return false;
            }
        ...
        public final int compareTo(final PathArgument arg) {
            ...
            return argCaseType.map(aClass -> compareClasses(caseType.orElseThrow(), aClass)).orElseThrow();
        }
     ...
     */
    public static class IdentifiableItem<I extends KeyAware<K> & DataObject, K extends Key<I>>
        ...
        IdentifiableItem(final Class<I> type, final K key) {
            super(type);
         ...
         * @throws NullPointerException if any argument is null.
         */
        public static <L extends KeyAware<K> & DataObject, K extends Key<L>>
                @NonNull IdentifiableItem<L, K> of(final Class<L> type, final K key) {
         ...
         * @throws NullPointerException if any argument is null.
         */
        public static <C extends ChoiceIn<?> & DataObject, T extends ChildOf<? super C> & KeyAware<K>,
                K extends Key<T>> @NonNull IdentifiableItem<T, K> of(final Class<C> caseType, final Class<T> type,
                    final K key) {
    ...
    public interface InstanceIdentifierBuilder<T extends DataObject> {
         ...
         * @throws NullPointerException if any argument is null
         */
        <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>>
         ...
         * @throws NullPointerException if any argument is null
         */
        <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>,
            N extends KeyAware<K> & ChildOf<? super C>> @NonNull KeyedInstanceIdentifierBuilder<N, K> child(
                Class<C> caze, Class<N> listItem, K listKey);

        <N extends ChildOf<? super T> & KeyAware<?>> @NonNull WildcardBuilder<N> wildcardChild(Class<N> container);

        <C extends ChoiceIn<? super T> & DataObject, N extends ChildOf<? super C> & KeyAware<?>>
        ...
        <N extends DataObject & Augmentation<? super T> & KeyAware<?>> @NonNull WildcardBuilder<N> wildcardAugmentation(
            Class<N> container);
