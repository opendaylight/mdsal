...
public interface InstanceNotificationService extends BindingService {
    ...
    <P extends DataObject, N extends InstanceNotification<N, P>, T extends Listener<P, N>>
        @NonNull Registration registerListener(Class<N> type, DataTreeIdentifier<P> path, T listener,
            Executor executor);

    default <P extends DataObject, N extends InstanceNotification<N, P>, T extends Listener<P, N>>
        @NonNull Registration registerListener(final Class<N> type, final DataTreeIdentifier<P> path,
            final T listener) {
        return registerListener(type, path, listener, MoreExecutors.directExecutor());
    }
    ...
    <P extends DataObject & Identifiable<K>, N extends KeyedListNotification<N, P, K>, K extends Identifier<P>,
        T extends KeyedListListener<P, N, K>> @NonNull Registration registerListener(Class<N> type,
            DataTreeIdentifier<P> path, T listener, Executor executor);

    default <P extends DataObject & Identifiable<K>, N extends KeyedListNotification<N, P, K>, K extends Identifier<P>,
        T extends KeyedListListener<P, N, K>> @NonNull Registration registerListener(final Class<N> type,
