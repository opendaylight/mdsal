...
class WildcardBuilderImpl<T extends DataObject> implements WildcardBuilder<T> {
    ...
    protected <N extends DataObject & KeyAware<K>, K extends Key<N>>
            KeyedWildcardBuilderImpl<N, K> getKeyedWildcardBuilder(final K key) {
    ...
    @Override
    public <N extends KeyAware<K> & ChildOf<? super T>, K extends Key<N>> @NonNull KeyedWildcardBuilder<N, K> child(
    ...
    public <C extends ChoiceIn<? super T> & DataObject, K extends Key<N>, N extends KeyAware<K> & ChildOf<? super C>>
            @NonNull KeyedWildcardBuilder<N, K> child(final Class<C> caze, final Class<N> listItem, final K listKey) {
    ...
    @SuppressWarnings("unchecked")
    <N extends DataObject & KeyAware<K>, K extends Key<N>> @NonNull KeyedWildcardBuilderImpl<N, K> addNode(
            final IdentifiableItem<?,?> newArg) {
        addNodeInternal(newArg);
