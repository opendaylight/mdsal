...
public abstract class AbstractDOMServiceTestKit<D extends DOMDataBroker> extends EffectiveModelContextTestKit
    ...
    public static class DTCLPolicy {
        ...
        protected static final class Capture extends DTCLPolicy {
    ...
    @FunctionalInterface
    @NonNullByDefault
    public interface DTCLClassifier {
    ...
    final DTCLClassifier classifier = (treeId, listener) -> DTCLPolicy.PASS;

    private volatile DOMNotificationRouter domNotificationRouter;
    private volatile FilteringDOMDataBroker<D> domDataBroker;
    ...
    public final DOMDataBroker domDataBroker() {
        FilteringDOMDataBroker local = domDataBroker;
        if (local == null) {
            synchronized (this) {
                local = domDataBroker;
                if (local == null) {
    ...
    private @NonNull FilteringDOMDataBroker<D> createDomDataBroker() {
        ...
        return new FilteringDOMDataBroker<>(createDomDataBroker(datastores), classifier);
    }
}
