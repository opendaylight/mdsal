...
public final class ReferenceProcessor extends AbstractProcessor {
    ...
    private final List<ReferenceSite> references = new ArrayList<>();
    ...
    private void processRound(final Set<? extends TypeElement> annotations, final RoundEnvironment roundEnv) {
        ...
        for (var annotation : annotations) {
            ...
            if (fqn.contentEquals(INITIAL_CONFIGURATION)) {
                processInitialConfiguration(roundEnv.getElementsAnnotatedWith(annotation));
            } else if (fqn.contentEquals(REFERENCE)) {
                processReference(roundEnv.getElementsAnnotatedWith(annotation));
            } else {
        ...
        for (var reference : references) {
            notez("Needz shinie " + reference.annotatedElement);
        }
    ...
    private void processInitialConfiguration(final Set<? extends Element> elements) {
    ...
    private void processReference(final Set<? extends Element> elements) {
        for (var element : elements) {
    ...
    private void processReference(final Element element) {
        ...
        if (typeName.contentEquals(CONFIGURATION_SNAPSHOT)) {
            ...
            addConfigurationReference(type, element);
            ...
            if (typeArgs.size() == 1) {
                ...
                if (firstName.contentEquals(CONFIGURATION_SNAPSHOT)) {
                    addConfigurationReference(firstArg, element);
                }
            }
        }
    }

    private void addConfigurationReference(final DeclaredType cfgSnapType, final @NonNull Element element) {
    ...
    private void emitDescriptor() throws IOException {
        ...
        // FIXME: output all constants

        final String xmlBody = sb.append("</cfg:initial>").toString();
        try {
            DescriptorConstants.descriptorSchema().newValidator().validate(new StreamSource(new StringReader(xmlBody)));
        ...
        final var file = processingEnv.getFiler().createResource(StandardLocation.CLASS_OUTPUT, "",
            DescriptorConstants.METAINF_SCR_CONFIGURATION);
