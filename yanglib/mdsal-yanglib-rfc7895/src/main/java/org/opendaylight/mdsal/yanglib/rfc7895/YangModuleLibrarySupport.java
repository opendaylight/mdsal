...
public final class YangModuleLibrarySupport implements YangLibSupport {
    private final BindingDataObjectCodecTreeNode<ModulesState> moduleStateCodec;
    private final EffectiveModelContext context;

    private YangModuleLibrarySupport(final EffectiveModelContext context,
            final BindingDataObjectCodecTreeNode<ModulesState> moduleStateCodec) {
        this.context = requireNonNull(context);
        this.moduleStateCodec = requireNonNull(moduleStateCodec);
    }

    public static YangLibSupport create(final YangParserFactory parserFactory) throws YangSyntaxErrorException,
    ...
    public MountPointContextFactory createMountPointContextFactory(final MountPointIdentifier mountId,
        ...
        return new MountPointContextFactoryImpl(mountId, resolver, context, moduleStateCodec);
    }
    ...
    private static YangTextSchemaSource createSource(final YangModuleInfo info) {
        return YangTextSchemaSource.delegateForByteSource(
            RevisionSourceIdentifier.create(info.getName().getLocalName(), info.getName().getRevision()),
