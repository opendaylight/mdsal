...
public class LegacyYangLibraryFormatTest {
    ...
    public void testLegacyFormat() {
        ...
        final List<? extends ContainerNode> libraryContent =
                yangLib.newContentBuilder()
                        .setContext(runtimeContext.getEffectiveModelContext())
                        .includeLegacy()
                        .formatYangLibraryContent();

        assertEquals(2, libraryContent.size());

        // content has stable order so we can just grab the legacy content and check that here
        final ModulesState modulesState = legacyCodec.deserialize(libraryContent.get(1));

        assertEquals(4, modulesState.getModule().size());
    ...
    private static Module createModule(String name, String namespace, String revision) {
