...
public class YangLibrarySupportTest {
    ...
    public void testFormatSchema() {
        ...
        final List<? extends ContainerNode> nonLegacyContent = yangLib.newContentBuilder()
                .setContext(runtimeContext.getEffectiveModelContext()).formatYangLibraryContent();

        assertEquals(1, nonLegacyContent.size());

        final YangLibrary yangLibrary = codec.deserialize(nonLegacyContent.get(0));

        assertEquals(1, yangLibrary.getModuleSet().size());
        final ModuleSet moduleSet = yangLibrary.getModuleSet().get(new ModuleSetKey("ODL_modules"));
        assertEquals(4, moduleSet.getModule().size());
    ...
    private Module createModule(String name, String namespace, String revision) {
