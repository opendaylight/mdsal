...
final class ClusterSingletonServiceGroupImpl<P extends Path<P>, E extends GenericEntity<P>,
    ...
    private volatile GroupState targetState;
    ...
    void initialize() throws CandidateAlreadyRegisteredException {
        verify(tryLock());
        try {
            checkState(targetState == null, "Singleton group %s was already initilized", identifier);
            ...
            synchronized (this) {
                ...
                targetState = GroupState.INACTIVE;
                endCapture();
            }
        } finally {
            unlock();
    ...
    void registerService(final ClusterSingletonServiceRegistration reg) {
        ...
        checkState(targetState != null, "Service group %s is not initialized yet", identifier);
    ...
    @GuardedBy("this")
    private @NonNull GroupState calculateTargetState() {
        if (cleanupEntityState != EntityState.OWNED) {
            return GroupState.INACTIVE;
        }
