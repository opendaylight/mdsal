...
final class ClusterSingletonServiceGroupImpl<P extends Path<P>, E extends GenericEntity<P>,
    ...
    private void lockedClose() {
        switch (state) {
            ...
            case OWNER:
                ...
                // FIXME: transition to STOPPING_SERVICES
    ...
    public void initializationClusterSingletonGroup() throws CandidateAlreadyRegisteredException {
        ...
        lock.lock();
        try {
            Preconditions.checkState(state == State.INITIAL, "Unexpected singleton group %s state %s",
                identifier, state);
    ...
    private void startServices() {
        ...
        serviceGroup.forEach(service -> {
            ...
            try {
                ...
                LOG.warn("Service group {} service {} failed to start, attempting to continue",
                    identifier, service, e);
            }
        });
            ...
            case OWNER:
                ...
                for (final ClusterSingletonServiceRegistrationDelegator service : serviceGroup) {
                    ...
                    try {
                        ...
                        LOG.warn("Service group {} service {} failed to stop, attempting to continue",
                            identifier, service, e);
                        continue;
                    }
