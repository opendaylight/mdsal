...
final class ClusterSingletonServiceGroupImpl<P extends Path<P>, E extends GenericEntity<P>,
    ...
    private void reconcileState() {
        while (true) {
            ...
            // We are about to check state: mark the observation point. If a future completes after this point,
            // we need to retry
            recheck = 0;
            reconcileState(localConfig);

            // We have ran a round of reconciliation, but the either registration/unregistration may have happened
            // or a callback needing attention may have gone by. Check that before we release the lock and also after,
            // so we do not introduce an unnecessary race window (due to dropped lock) but at the same time guard
            // against race where:
            //
            ...
            if (needReconcile(stamp)) {
                LOG.debug("Service group {} needs reconciliation", identifier);
                continue;
            }
            applyLock.release();
            if (!needReconcile(stamp)) {
    ...
    private boolean needReconcile(final long stamp) {
        if (recheck != 0) {
            LOG.debug("Service group {} needs recheck due to a future completing", identifier);
            return true;
        }
        if (configLock.validate(stamp)) {
            return false;
        ...
        LOG.debug("Service group {} needs recheck due a service having been added/removed", identifier);
        return true;
    }
