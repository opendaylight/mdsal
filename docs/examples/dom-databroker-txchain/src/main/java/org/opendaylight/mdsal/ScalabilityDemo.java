...
public final class ScalabilityDemo {
    ...
    private static void run(final DOMDataBroker broker) throws InterruptedException {
        ...
        DOMDataTreeChangeListener listener = new DOMDataTreeChangeListener() {
            @Override
        ...
        try (var txChain = broker.createMergingTransactionChain()) {
            for (int i = 0; i < ITERATIONS; ++i) {
                ...
                tx.commit().addCallback(new FutureCallback<CommitInfo>() {
                    ...
                    public void onFailure(final Throwable cause) {
                        LOG.warn("Failed to commit", cause);
        ...
        System.out.printf("MergeChain Total tx:%d  Success tx:%d  Failure tx:%d  Elapsed:%d elapsed ms  TPS:%.1f%n",
            ITERATIONS, successfulTx.get(), failedTx.get(), elapsed, (ITERATIONS * 1_000.0) / elapsed);
    }
}
