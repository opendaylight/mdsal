...
final class GenHelperUtil {
     ...
     *
     * @param keyPackageName
     *            string contains the key package name
     * @param basePackageName
     *            string contains the module package name
     * @param schemaNode
    ...
    private static void addSchemaNodeToListBuilders(final String nodeName, final String basePackageName,
            ...
            final GeneratedTOBuilder genTOBuilder, final List<String> listKeys, final Module module,
        ...
        if (schemaNode instanceof LeafSchemaNode) {
            ...
            if (listKeys.contains(leafName)) {
                leafPckgName = new StringBuilder(leafPckgName).append(".key.").append(nodeName).toString();
                isKeyPart = true;
            } else {
                leafPckgName = new StringBuilder(leafPckgName).append(".data.").append(nodeName).toString();
            }

            final GeneratedTypeBuilder leafGTp = new GeneratedTypeBuilderImpl(leafPckgName, leafName);
            resolveLeafSchemaNodeAsMethod(schemaContext, leafGTp, genCtx, leaf, module,
                    typeProvider);

            constructGetter(typeBuilder, leafName, schemaNode.getDescription(), leafGTp, Status.CURRENT);
