...
final class GenHelperUtil {
    ...
    static Map<Module, ModuleContext> processUsesImplements(final DataNodeContainer node,
            final Module module, Map<Module, ModuleContext> genCtx) {

        for (final UsesNode usesNode : node.getUses()) {
            Object groupingNode =  genCtx.get(module).getTypeToSchema()
                    .get(findGroupingByPath(usesNode.getGroupingPath(), genCtx));
            ...
            for (SchemaNode grpChildNode : grouping.getChildNodes()) {
                if (grpChildNode instanceof DataNodeContainer) {
                    DataSchemaNode childNode = node.getDataChildByName(grpChildNode.getQName());
                    genCtx.get(module).getChildNode(childNode.getPath())
                            .addImplementsType(genCtx.get(module).getChildNode(grpChildNode.getPath()));
                }
            }
        }
        return genCtx;
    ...
    private static void containerToGenType(final Module module, final String basePackageName,
        ...
        if (genType != null) {
            ...
            resolveDataSchemaNodes(module, basePackageName, genType, genType, node.getChildNodes(), genCtx,
                    schemaContext, verboseClassComments, genTypeBuilders, typeProvider, namespaceType);
            processUsesImplements(node, module, genCtx);
        }
    }
    ...
    private static Map<Module, ModuleContext> groupingToGenType(final String basePackageName, final GroupingDefinition grouping, final Module
        ...
        genCtx = processUsesImplements(grouping, module, genCtx);
        return genCtx;
    }
