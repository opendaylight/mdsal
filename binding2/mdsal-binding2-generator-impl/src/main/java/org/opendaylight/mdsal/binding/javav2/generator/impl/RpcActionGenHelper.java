...
public final class RpcActionGenHelper {
    ...
    private static GeneratedTypeBuilder resolveOperation(final DataSchemaNode parent, final OperationDefinition operation,
        ...
        final MethodSignatureBuilder outputMethod = inType.addMethod("implementedInterface");
        ...
        if (parent instanceof ListSchemaNode) {
            //TODO: finish list Action
            interfaceBuilder.addImplementsType(parameterizedTypeFor(LIST_ACTION, inType, outType));
        } else {
            if (isAction) {
