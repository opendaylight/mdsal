...
/**
 *
 * Util class used for generation of types for RPCs, routedRPCs and Actions (YANG 1.0 only1)
        ...
        for (DataSchemaNode potential : potentials) {
            if (potential instanceof ActionNodeContainer) {
                ...
                for (ActionDefinition action: actions) {
                    ...
                    final String fullyQualifiedName = basePackageName + "." + BindingMapping.getClassName(action.getQName());
     ...
     static Map<Module, ModuleContext> rpcMethodsToGenType(final Module module, Map<Module, ModuleContext> genCtx,
        ...
        for (final RpcDefinition rpc : rpcDefinitions) {
            if (rpc != null) {
                ...
                interfaceBuilder.setDescription(createDescription(rpc, basePackageName + "." + BindingMapping
                        .getClassName(rpc.getQName()), schemaContext,
                        verboseClassComments));
