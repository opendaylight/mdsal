...
/**
 * Util class for fix of bad form of generated java file
 */
public final class JavaFileCleaner {
    ...
    private JavaFileCleaner() {
        throw new UnsupportedOperationException("Util class");
    }

    /**
     * Fix generated java file.
     *
     * @param unformedJavaFile
     *            - unformed java file from generator
     * @return formed java file
     */
    public static String clean(final String unformedJavaFile) {
    ...
    private static int lineIndent(final StringBuilder sb, final int indentCount, final String line) {
        ...
        if (line.contains(String.valueOf(END_BODY))) {
            newIndentCount--;
        }
        ...
        if (line.contains(String.valueOf(START_BODY))) {
            newIndentCount++;
        }
    ...
    /**
     * Merge or separate lines, which need it.
     *
     * @param splittedByNewLine
        ...
        if (splittedByNewLine[i].contains(JDOC_START)) {
            ...
            for (int j = i + 1; j < splittedByNewLine.length - 1; j++) {
                if (splittedByNewLine[j].contains(JDOC_END)) {
                    fixedLines.add(new StringBuilder(SPACE).append(SPACE).append(splittedByNewLine[j]).append(NEW_LINE)
                            .toString());
                    i = j + 1;
                    break;
                } else {
    ...
    /**
     * Remove empty lines and whitespaces around lines.
     *
     * @param splittedByNewLine
