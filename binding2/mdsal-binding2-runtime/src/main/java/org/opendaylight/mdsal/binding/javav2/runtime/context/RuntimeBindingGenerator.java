...
public final class RuntimeBindingGenerator {
    ...
    RuntimeBindingGenerator(final SchemaContext context) {
        requireNonNull(context, "Schema Context reference cannot be NULL.");
    ...
    public BiMap<Type, WithStatus> getTypeToSchemas() {
    ...
    private void generateAugmentationTypes(final Module module) {
        module.getAugmentations().stream()
            ...
            .collect(Collectors.groupingBy(AugmentationSchemaNode::getTargetPath)).entrySet()
    ...
    private void generateAugmentationType(final Module module, final Map.Entry<SchemaPath, List<AugmentationSchemaNode>> entry)  {
        ...
        entry.getValue().forEach(aug -> generateDataNodeTypes(module,
    ...
    private Type newType(final String normalizedPackageName, final String rawTypeName) {
        return new ReferencedTypeImpl(normalizedPackageName, rawTypeName, true, false);
    }
