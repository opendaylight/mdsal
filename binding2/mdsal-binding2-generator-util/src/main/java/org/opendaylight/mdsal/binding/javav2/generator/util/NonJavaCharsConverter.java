 ...
 * </ul>
 * </li>
 * <li>package - part of package name
 * (<a href="https://docs.oracle.com/javase/tutorial/java/package/namingpkgs.html"> Naming a
 * package</a>)</li>
 * <li>
 * <li>
 * <ul>
 * <li>without special separator for part of package name</li>
...
public final class NonJavaCharsConverter {
    ...
    // _foo_ -> _foo_
    // foo-boo -> foo_boo
    private static final String UNDERSCORE_ACCEPTING_MARKER = "UNDERSCOREACCEPTINGMARKER";
     ...
     */
    public static String normalizePackageName(final String packageNamePart) {
        StringBuilder normalizedName = new StringBuilder(packageNamePart);
        if (BindingMapping.JAVA_RESERVED_WORDS.contains(packageNamePart) || BindingMapping.WINDOWS_RESERVED_WORDS.contains(packageNamePart)) {
            return normalizedName.append(UNDERSCORE).toString();
        }
        if (packageNamePart.contains(String.valueOf(UNDERSCORE))) {
            normalizedName =
                    new StringBuilder(packageNamePart.replaceAll(String.valueOf(UNDERSCORE), UNDERSCORE_ACCEPTING_MARKER));
        }
        if (packageNamePart.contains(String.valueOf(DASH))) {
            normalizedName =
                    new StringBuilder(
                            packageNamePart.replaceAll(String.valueOf(DASH), String.valueOf(UNDERSCORE_ACCEPTING_MARKER)));
        }
    ...
    public static String convertIdentifier(final String identifier, final JavaIdentifier javaIdentifier) {
        ...
        if(javaIdentifier != JavaIdentifier.PACKAGE) {
            if (BindingMapping.JAVA_RESERVED_WORDS.contains(identifier)
                    || BindingMapping.WINDOWS_RESERVED_WORDS.contains(identifier)) {
                return fixCasesByJavaType(sb.append(identifier).append(UNDERSCORE).append(RESERVED_KEYWORD).toString(),
                        javaIdentifier);
            }
        }
        ...
        switch (javaIdentifier) {
            ...
            case PACKAGE:
                return convertedIdentifier.replaceAll(String.valueOf(UNDERSCORE), EMPTY_STRING)
                        .replaceAll(UNDERSCORE_ACCEPTING_MARKER.toLowerCase(), String.valueOf(UNDERSCORE));
            default:
