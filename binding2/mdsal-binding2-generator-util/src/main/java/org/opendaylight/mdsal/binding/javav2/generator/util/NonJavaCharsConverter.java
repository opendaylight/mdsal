...
public final class NonJavaCharsConverter {
    ...
    public static String normalizePackageName(final String packageNamePart) {
        StringBuilder normalizedNameSb = new StringBuilder(packageNamePart);
        if (BindingMapping.JAVA_RESERVED_WORDS.contains(packageNamePart)
                || BindingMapping.WINDOWS_RESERVED_WORDS.contains(packageNamePart.toUpperCase())) {
            return normalizedNameSb.append(UNDERSCORE).toString();
        }
        ...
        normalizedNameSb = new StringBuilder();
        ...
        for (int i = 0; i < normalizedPackageNamePart.length(); i++) {
            if (normalizedPackageNamePart.charAt(i) == UNDERSCORE) {
                if (!innserSb.toString().isEmpty()) {
                    normalizedNameSb.append(convertIdentifier(innserSb.toString(), JavaIdentifier.PACKAGE));
                    innserSb = new StringBuilder();
                }
                normalizedNameSb.append(UNDERSCORE);
            } else {
