...
public abstract class AbstractForwardedTransaction<
    ...
    protected final <D extends TreeNode> CheckedFuture<Optional<D>, ReadFailedException> doRead(
        ...
        return MappingCheckedFuture.create(Futures.transform(
            Futures.transform(readTx.read(store, codec.toYangInstanceIdentifierBlocking(path)),
                Optional::fromJavaUtil, MoreExecutors.directExecutor()),
            codec.deserializeFunction(path), MoreExecutors.directExecutor()), ReadFailedException.MAPPER);
    }
}
