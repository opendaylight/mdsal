 ...
 * Copyright (c) 2026 SmartOptics AS
 *
...
final class TreeReplica extends ChannelInitializer<SocketChannel> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TreeReplica.class);
    ...
    TreeReplica(NettyReplicationConfig config, DOMDataTreeIdentifier tree) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Failed to create subscribe request for tree " + tree, e);
        }
    }
    ...
    synchronized boolean stop() {
        ...
        if (channel != null && channel.isActive()) {
            try {
                ...
                LOG.error("The channel for tree {} didn't close properly within {} seconds", tree,
    ...
    private void doConnect() {
        ...
        futureChannel = bootstrap.option(ChannelOption.SO_KEEPALIVE, true).handler(this)
    ...
    protected void initChannel(final SocketChannel ch) {
        ...
        ch.pipeline()
                ...
                .addLast("idleStateHandler",
                        new IdleStateHandler(keepaliveInterval.toNanos() * maxMissedKeepalives, 0, 0,
        ...
        txChain.addCallback(new FutureCallback<>() {
            ...
            public void onSuccess(final Empty result) {
                LOG.info("Transaction chain for channel {} and tree {} completed", ch, tree);
            }

            @Override
            public void onFailure(final Throwable cause) {
                LOG.error("Transaction chain for channel {} and tree {} failed", ch, tree, cause);
                ch.close();
            }
        });
    }
    ...
    private synchronized void channelResolved(final ChannelFuture completedFuture,
        ...
        if (futureChannel != null && futureChannel.channel() == completedFuture.channel()) {
            if (completedFuture.isSuccess()) {
                ...
                LOG.info("Failed to connect to source {} for tree {}, reconnecting in {}", sourceAddress, tree,
    ...
    private synchronized void channelClosed(final ChannelFuture completedFuture, final ScheduledExecutorService group) {
        if (futureChannel != null && futureChannel.channel() == completedFuture.channel() && !isClosingInstance()) {
            LOG.info("Channel {} lost connection to source {} for tree {}, reconnecting in {}",
                    completedFuture.channel(), sourceAddress, tree, reconnectDelay.getSeconds());
            group.schedule(this::reconnect, reconnectDelay.toNanos(), TimeUnit.NANOSECONDS);
        }
    }
