...
final class SinkSingletonService extends ChannelInitializer<SocketChannel> implements ClusterSingletonService {
    ...
    SinkSingletonService(final BootstrapSupport bootstrapSupport, final DOMDataBroker dataBroker,
        ...
        Preconditions.checkArgument(maxMissedKeepalives > 0, "max-missed-keepalives must be greater then 0");
        this.maxMissedKeepalives = maxMissedKeepalives;
        this.closingInstance = false;
    ...
    protected void initChannel(final SocketChannel ch) {
        ch.pipeline()
            ...
            .addLast("idleStateHandler", new IdleStateHandler(
                keepaliveInterval.getSeconds() * maxMissedKeepalives, 0, 0, TimeUnit.SECONDS))
    ...
    private synchronized void channelResolved(final ChannelFuture completedFuture, final ScheduledExecutorService group) {
        if (futureChannel.channel() == completedFuture.channel()) {
            ...
            LOG.warn("ChannelResolved called for channel {}, but state channel is {}", completedFuture.channel(),
    ...
    private synchronized void channelClosed(final ChannelFuture completedFuture, final ScheduledExecutorService group) {
        if (futureChannel != null && futureChannel.channel() == completedFuture.channel()) {
            ...
            LOG.warn("channelClosed called for channel {}, but state channel is {}", completedFuture.channel(),
                futureChannel.channel());
        }
    }
