...
public class IntegrationTest extends AbstractDataBrokerTest {
     ...
     */
    @Test
    public void testReplicateInitialState() throws InterruptedException {
        ...
        Assert.assertEquals(expectedEntityState, capturedInitialState);
    ...
    private NormalizedNode generateNormalizedNodeForEntities(final int amount) {
        CollectionNodeBuilder<MapEntryNode, MapNode> builder = ImmutableNodes.mapNodeBuilder(ENTITY_QNAME);
    ...
    private void generateModification(final DataBroker broker, final int amount) {
        for (int i = 0; i < amount; i++) {
