...
class TracingWriteTransaction implements DOMDataWriteTransaction {
    ...
    public TracingWriteTransaction(DOMDataWriteTransaction delegate, TracingBroker tracingBroker) {
        this.delegate = delegate;
        this.tracingBroker = tracingBroker;
    }

    private void recordOp(LogicalDatastoreType store, YangInstanceIdentifier iid,
        ...
        if (value != null && value instanceof ImmutableSet && ((Set)value).isEmpty()) {
            if (tracingBroker.LOG.isDebugEnabled()) {
                tracingBroker.LOG.debug("Empty data set write to {}", tracingBroker.toPathString(iid));
            }
        } else {
