...
class TracingReadWriteTransaction extends TracingWriteTransaction implements DOMDataReadWriteTransaction {
    ...
    public TracingReadWriteTransaction(DOMDataReadWriteTransaction delegate, TracingBroker tracingBroker) {
    ...
    /*
    @Override
    public <T extends DataObject> CheckedFuture<Optional<T>, ReadFailedException> read(LogicalDatastoreType store, InstanceIdentifier<T> instanceIdentifier) {
        return delegate.read(store, instanceIdentifier);
    }
    */
