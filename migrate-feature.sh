...
do
  ...
  # 4 Apply mustaches inside template.xml
    ...
    # 4.2 On <configfile> lines, use ${project.version} instead
    sed -E '/<configfile/ s#(mvn:[^/:]+/[^/]+/)[^/<"]+#\1${project.version}#g' "$dest" > "$dest.sed" && mv "$dest.sed" "$dest"

    # 4.3 For dependency features (lines without name):
    ...
    # 4.4 Clean top-level <feature name="...">: remove description= and version= attributes
    ...
    # 4.5 Remove any <details>…</details> lines
