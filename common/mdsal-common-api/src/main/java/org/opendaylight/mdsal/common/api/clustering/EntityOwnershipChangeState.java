...
public enum EntityOwnershipChangeState {
    ...
    private static final Map<Key, EntityOwnershipChangeState> BY_KEY = new HashMap<>();
    static {
        for(EntityOwnershipChangeState e: values()) {
            BY_KEY.put(new Key(e.wasOwner, e.isOwner, e.hasOwner), e);
        }
    }
    ...
    public static EntityOwnershipChangeState from(boolean wasOwner, boolean isOwner, boolean hasOwner) {
        ...
        if(state == null) {
            throw new IllegalArgumentException(String.format(
                    "Invalid combination of wasOwner: %s, isOwner: %s, hasOwner: %s", wasOwner, isOwner, hasOwner));
        }
