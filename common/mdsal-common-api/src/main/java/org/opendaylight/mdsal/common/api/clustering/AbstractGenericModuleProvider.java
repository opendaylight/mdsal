...
/**
 * Abstract class {@link AbstractGenericModuleProvider} represents a single cluster project instance.
 ...
 */
public abstract class AbstractGenericModuleProvider<P extends Path<P>, E extends GenericEntity<P>,
    ...
    private static final Logger LOG = LoggerFactory.getLogger(AbstractGenericModuleProvider.class.getName());
     ...
     */
    protected AbstractGenericModuleProvider(@Nonnull final S entityOwnershipService) {
        this.entityOwnershipService = Preconditions.checkNotNull(entityOwnershipService);
        this.serviceEntityIdentifier = this.getClass().getCanonicalName();
    }
    ...
    public final void initializeProvider() {
        LOG.debug("Initialization method for project Provider {}", serviceEntityIdentifier);
        this.projectEntity = createEntity(PROJECT_ENTITY_TYPE, serviceEntityIdentifier);
        this.doubleCandidateEntity = createEntity(CLOSE_PROJECT_ENTITY_TYPE, serviceEntityIdentifier);
        ...
        try {
            ...
            LOG.debug("Unexpected error by registration project Provider {}", serviceEntityIdentifier, e);
    ...
    public final void ownershipChanged(final C ownershipChange) {
        ...
        try {
            ...
            LOG.error("Unexpected Exception for project Provider {}", serviceEntityIdentifier, e);
    ...
    public final void close() {
        LOG.debug("Close method for project Provider {}", serviceEntityIdentifier);
        boolean needReleaseLock = false;
        try {
            needReleaseLock = clusterLock.tryAcquire(10, TimeUnit.SECONDS);
        } catch (final Exception e) {
            LOG.warn("Unexpected Exception for project Provider {} in closing phase.", serviceEntityIdentifier, e);
        } finally {
    ...
    private FutureCallback<Void> newAsyncCloseCallback(@Nullable final Semaphore semaphore) {
        final Consumer<Throwable> closeEntityCandidateRegistration = (@Nullable final Throwable t) -> {
            if (t != null) {
                LOG.warn("Unexpected error closing project instance {}", serviceEntityIdentifier, t);
            } else {
                LOG.debug("Destroy project Instance {} is success", serviceEntityIdentifier);
            }
    ...
    private void tryToTakeOwnership() {
        LOG.debug("TryToTakeLeadership method for project Provider {}", serviceEntityIdentifier);
        boolean needReleaseLock = false;
        boolean needCloseProviderInstance = false;
        try {
            clusterLock.acquire();
    ...
    private void takeOwnership() {
        LOG.debug("TakeLeadership method for project Provider {}", serviceEntityIdentifier);
        boolean needReleaseLock = false;
        boolean needCloseProviderInstance = false;
        try {
            clusterLock.acquire();
            ...
            LOG.error("Unexpected exception state for project Provider {} in TakeLeadership", serviceEntityIdentifier,
                    e);
            needCloseProviderInstance = true;
        } finally {
            closeResources(needReleaseLock, needCloseProviderInstance);
    ...
    private void lostOwnership() {
        LOG.debug("LostLeadership method for project Provider {}", serviceEntityIdentifier);
        boolean needReleaseLock = false;
        boolean needCloseProviderInstance = false;
        try {
            clusterLock.acquire();
             ...
             * Semaphore lock release has to be realized as FutureCallback after a project
             * instance has fully closed prior to relinquishing project ownership.
             */
            needReleaseLock = false;
        } catch (final Exception e) {
            LOG.error("Unexpected exception state for project Provider {} in LostLeadership", serviceEntityIdentifier,
                    e);
            needCloseProviderInstance = true;
        } finally {
            closeResources(needReleaseLock, needCloseProviderInstance);
