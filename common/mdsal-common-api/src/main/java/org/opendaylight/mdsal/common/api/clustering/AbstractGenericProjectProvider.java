 ...
 * There is only one fully instantiated project instance in a cluster at one time. So we will have
 * only one full instantiated project in whole cluster. Single instance This is realized via a double
 * candidate approach where a project instance maintains a candidate registration for ownership of the
 * project entity in the cluster and also a registration that acts as a guard to ensure a project
...
public abstract class AbstractGenericProjectProvider<P extends Path<P>, E extends GenericEntity<P>,
    ...
    private final R asyncCloseEntityListenerReg;
            ...
            if (projectEntity.equals(ownershipChange.getEntity())) {
                ...
                if (EntityOwnershipChangeState.LOCAL_OWNERSHIP_GRANTED.equals(ownershipChange.getState())) {
                    ...
                    takeOwnership();
                        ...
                        || EntityOwnershipChangeState.LOCAL_OWNERSHIP_LOST_NO_OWNER.equals(ownershipChange.getState())) {
                    /* MASTER to SLAVE : !ownershipChange.getState().isOwner() && ownershipChange.getState().wasOwner() */
                    LOG.error("Unexpected lost doubleCandidate {} leadership. Close project instance {}",
