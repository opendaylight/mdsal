 ...
 * sentence, it's redundant from the first sentence. All registered services have only one
 * instantiated service instance in a cluster at one time on same Cluster Node. This is
 * realized via a double candidate approach where a service group instance maintains
 * a candidate registration for ownership of the service entity in the cluster and
 * also a registration that acts as a guard to ensure a service group instance has
 * fully closed prior to relinquishing service ownership. To achieve ownership
 * of the service group, a service group candidate must hold ownership
