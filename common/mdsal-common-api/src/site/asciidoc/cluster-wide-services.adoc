...
Applications or services could be split for two base groups in cluster:

* apps instantiated per cluster node (e.g. OpenflowPlugin)
* apps instantiated once per cluster node suite (e.g. TopologyManager)

Applications or services which need to be instantiated per cluster only once has base problem. Which node is the right one:
...
First step has to focus for an easy generally applicable solution and in next we are able to focus to solve additional problems for a choice the right one clusterNode.
