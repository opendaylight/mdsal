...
public abstract class AbstractDOMStoreTreeChangePublisher
    ...
    protected final void processCandidateTree(@Nonnull final DataTreeCandidate candidate) {
        ...
        try (final RegistrationTreeSnapshot<AbstractDOMDataTreeChangeListenerRegistration<?>> snapshot
            ...
            final Map<AbstractDOMDataTreeChangeListenerRegistration<?>, Collection<DataTreeCandidate>>
                    listenerChanges = new IdentityHashMap<>();
    ...
    private void lookupAndNotify(final List<PathArgument> args,
            ...
            final Map<AbstractDOMDataTreeChangeListenerRegistration<?>,
                      Collection<DataTreeCandidate>> listenerChanges) {
        if (args.size() != offset) {
    ...
    private void addToListenerChanges(final Collection<AbstractDOMDataTreeChangeListenerRegistration<?>> registrations,
        ...
        for (AbstractDOMDataTreeChangeListenerRegistration<?> reg : registrations) {
            Collection<DataTreeCandidate> changes = listenerChanges.get(reg);
            if (changes == null) {
                changes = new ArrayList<>();
                listenerChanges.put(reg, changes);
            }

            changes.add(dataTreeCandidate);
        }
    }
}
