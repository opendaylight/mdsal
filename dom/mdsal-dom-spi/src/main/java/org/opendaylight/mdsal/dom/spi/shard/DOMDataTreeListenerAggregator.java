...
public final class DOMDataTreeListenerAggregator {
    ...
    private static final class CandidateQueue implements Identifiable<DOMDataTreeIdentifier> {
        ...
        synchronized void clearAndAppend(final Collection<DataTreeCandidate> candidate) {
            queue.clear();
            queue.addAll(candidate);
        }

        synchronized DataTreeState collect() {
            final List<DataTreeCandidate> ret = ImmutableList.copyOf(queue);
            queue.clear();
            return new DataTreeState(ret, null);
        }

        static CandidateQueue create(final DOMDataTreeIdentifier treeId, final boolean allowRxMerges) {
    ...
    private static final class Operational extends Behavior<DataTreeState> {
        ...
        private void notifyListener() {
            ...
            for (CandidateQueue queue : queues) {
                final DataTreeState state = queue.collect();
                subtrees.putAll(state.subtrees);
            ...
            if (size != 0) {
                ...
                listener.onDataTreeChanged(changes, subtrees.build());
