...
public abstract class AbstractStateAggregator<S extends AbstractStateAggregator.State> {
    ...
    private static final class Starting<S extends State> extends Behavior<Starting<S>, S> {
        ...
        final void add(final StateBuilder<S> builder) {
        ...
        @Override
        final synchronized void receiveState(final StateBuilder<S> builder, final S state) {
            if (successor != null) {
        ...
        final synchronized Started<S> start(final Function<Collection<StateBuilder<S>>, Started<S>> function) {
    ...
    protected static final class Failed<S extends State> extends Started<S> {
        public Failed(final Collection<? extends StateBuilder<S>> builders) {
            super(builders);
        }
    ...
    protected abstract static class Operational<S extends State> extends Started<S> {
        // Locking is a combination of a generation counter and a semaphore.
    ...
    protected synchronized final Started<S> start(final Function<Collection<StateBuilder<S>>, Started<S>> function) {
