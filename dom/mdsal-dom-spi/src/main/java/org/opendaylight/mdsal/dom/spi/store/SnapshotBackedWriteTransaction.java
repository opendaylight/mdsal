...
public class SnapshotBackedWriteTransaction<T> extends AbstractDOMStoreTransaction<T>
    ...
    public abstract static class TransactionReadyPrototype<T> {
        ...
        protected abstract DOMStoreThreePhaseCommitCohort transactionReady(SnapshotBackedWriteTransaction<T> tx,
            DataTreeModification tree, @Nullable Exception readyError);
    }

    private static final Logger LOG = LoggerFactory.getLogger(SnapshotBackedWriteTransaction.class);

    private static final VarHandle READY_VH;
    private static final VarHandle TREE_VH;

    static {
        final var lookup = MethodHandles.lookup();
        try {
            READY_VH = lookup
                .findVarHandle(SnapshotBackedWriteTransaction.class, "readyImpl", TransactionReadyPrototype.class);
            TREE_VH = lookup
                .findVarHandle(SnapshotBackedWriteTransaction.class, "mutableTree", DataTreeModification.class);
    ...
    private void checkNotReady() {
        if (readyImpl == null) {
            throw new IllegalStateException(
                "Transaction %s is no longer open. No further modifications allowed.".formatted(getIdentifier()));
        }
    }
    ...
    public DOMStoreThreePhaseCommitCohort ready() {
        final var wasReady = (TransactionReadyPrototype<T>) READY_VH.getAndSet(this, null);
        if (wasReady == null) {
            throw new IllegalStateException("Transaction %s is no longer open".formatted(getIdentifier()));
        }

        LOG.debug("Store transaction: {} : Ready", getIdentifier());

        final var tree = mutableTree;
        TREE_VH.setRelease(this, null);
        try {
            tree.ready();
