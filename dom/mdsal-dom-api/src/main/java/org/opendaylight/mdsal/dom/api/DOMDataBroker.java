 ...
 * <ul>
 *   <li>Read-Only - allocated using {@link #newReadOnlyTransaction()}</li>
 *   <li>Write-Only - allocated using {@link #newWriteOnlyTransaction()}</li>
 * </ul>
 *
 * <p>These transactions provide a stable isolated view of data tree, which is guaranteed to be not affected by other
 * concurrent transactions, until transaction is committed.
 *
 * <p>For a detailed explanation of how transaction are isolated and how transaction-local changes are committed to
 * global data tree, see {@link DOMDataTreeReadTransaction}, {@link DOMDataTreeWriteTransaction} and
...
public interface DOMDataBroker extends DOMService<DOMDataBroker, DOMDataBroker.Extension>, DOMTransactionFactory {
    ...
    interface DataTreeChangeExtension extends Extension {
         ...
         *
         * <p>You are able to register for data change notifications for a subtree or leaf even if it does not exist.
         * You will receive notification once that node is created.
         *
         ...
         *
         * <p>You MUST explicitly unregister your listener when you no longer want to receive notifications. This is
         ...
         *
         * <p>The sole known implementation, the Akka-based datastore, defines the difference in terms of RAFT,
         * suspending even delivery when the RAFT leader is not local. Even when there may be valid use cases for this,
         * RAFT there is a storage backend whose lifecycle is disconnected from this object.
         *
     ...
     * no outstanding transaction.
     *
     * <p>Unlike {@link #createTransactionChain()}, the transaction chain returned by this method is allowed to merge
     ...
     *
     * <p>When transactions are merged, {@link DOMTransactionChain#newReadOnlyTransaction()} may actually be backed by
