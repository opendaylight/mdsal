/*
 * Copyright (c) 2019-2024 PANTHEON.tech, s.r.o. and others.  All rights reserved.
 *
...
final class PotassiumDataOutput {
    ...
    public void writeObject(final @NonNull Object value) throws IOException {
        switch (value) {
            case String str -> writeValue(str);
            case Boolean bool -> writeValue(bool);
            case Byte byteVal -> writeValue(byteVal);
            case Short shortVal -> writeValue(shortVal);
            case Integer intVal -> writeValue(intVal);
            case Long longVal -> writeValue(longVal);
            case Uint8 uint8 -> writeValue(uint8);
            case Uint16 uint16 -> writeValue(uint16);
            case Uint32 uint32 -> writeValue(uint32);
            case Uint64 uint64 -> writeValue(uint64);
            case QName qname -> writeValue(qname);
            case YangInstanceIdentifier id -> writeValue(id);
            case byte[] bytes -> writeValue(bytes);
            case Empty empty -> output.writeByte(PotassiumValue.EMPTY);
            case Set<?> set -> writeValue(set);
            case Decimal64 decimal -> {
    ...
    private void writeValue(final YangInstanceIdentifier value) throws IOException {
        final var args = value.getPathArguments();
    ...
    void writePathArgumentInternal(final PathArgument pathArgument) throws IOException {
        switch (pathArgument) {
            ...
            case YangInstanceIdentifier.NodeWithValue<?> niv -> writeNodeWithValue(niv);
            case null, default -> throw new IOException("Unhandled PathArgument " + pathArgument);
        }
    }
    ...
    private void writeNodeIdentifierWithPredicates(final NodeIdentifierWithPredicates identifier) throws IOException {
        final var size = identifier.size();
        if (size < 13) {
    ...
    private void encodeString(final @NonNull String str) throws IOException {
        final var code = stringCodeMap.get(str);
        if (code != null) {
            writeRef(code);
        } else {
