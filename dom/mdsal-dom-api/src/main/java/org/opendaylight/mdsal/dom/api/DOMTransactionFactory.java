 ...
 *
 * <p><b>Note:</b> This interface is not intended to be used directly, but rather via subinterfaces which introduce
 * additional semantics to allocated transactions.
 * <ul>
 *   <li>{@link DOMDataBroker}</li>
 *   <li>{@link DOMTransactionChain}</li>
 * </ul>
 *
...
public interface DOMTransactionFactory {
     ...
     * transaction will fail. See {@link DOMDataTreeWriteTransaction#commit()} for more details about conflicting and
     * non-conflicting changes and failure scenarios.
     *
     * <p>Since this transaction does not provide a view of the data it SHOULD BE used only by callers which are
     * exclusive writers (exporters of data) to the subtree they modify. This prevents optimistic lock failures as
     * described in {@link DOMDataTreeWriteTransaction#commit()}.
     *
     ...
     * <p>Preconditions for mutation of data tree are captured from the snapshot of data tree state, when the
     * transaction is allocated. If data was changed during transaction in an incompatible way then the commit of this
     * transaction will fail. See {@link DOMDataTreeReadWriteTransaction#commit()} for more details about conflicting
     * and non-conflicting changes and failure scenarios.
     *
