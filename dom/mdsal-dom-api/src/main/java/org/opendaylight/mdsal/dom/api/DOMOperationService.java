 ...
 * service is that of a dynamic router, where the set of available Operation services can change
...
public interface DOMOperationService extends DOMService {
    ...
    void invokeAction(@Nonnull SchemaPath type, @Nonnull YangInstanceIdentifier parent,
            @Nonnull NormalizedNode<?, ?> input,  @Nonnull BiConsumer<DOMRpcResult, DOMRpcException> callback);

    /**
     * Register a {@link DOMOperationAvailabilityListener} with this service to receive notifications
     * about Operation implementations becoming (un)available. The listener will be invoked with the
