...
public final class DOMQueryPredicate implements Immutable {
    ...
    public abstract static class Match implements WritableObject {
        ...
        /**
         * Read a Match from a DataInput.
         * The format is expected to match the output format of {@link #writeTo(DataOutput)}.
         *
        ...
        public static @NonNull Match readFrom(final DataInput in) throws IOException {
            final var matcherType = in.readByte();
        ...
        static @NonNull Object readValue(final DataInput in) throws IOException {
    ...
    private static byte[] serialize(final Object obj) throws IOException {
        try (var b = new ByteArrayOutputStream()) {
            try (var stream = new DataOutputStream(b)) {
                var writer = new PotassiumDataOutput(stream);
    ...
    private static Object deserialize(final byte[] bytes) throws IOException {
        return new PotassiumDataInput(ByteStreams.newDataInput(bytes)).readObject();
    }
    ...
    private static final class MatchStringMatches extends AbstractMatch {
        ...
        public void writeTo(@NonNull final DataOutput out) throws IOException {
            ...
            out.writeUTF(pattern.pattern());
            out.writeInt(pattern.flags());
        }

        public static @NonNull Match readFrom(final DataInput in) throws IOException {
            var patter = in.readUTF();
            int flags = in.readInt();
            return Match.stringMatches(Pattern.compile(patter, flags));
        }
    ...
    private abstract static class CompositeMatch extends Match {
        ...
        static ImmutableList<Match> readComponents(final DataInput in) throws IOException {
