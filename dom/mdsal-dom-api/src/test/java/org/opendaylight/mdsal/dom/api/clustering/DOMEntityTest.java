...
public class DOMEntityTest {
    ...
    public void testEntityNameConstructor() {
        ...
        assertEquals("First PathArgument node type", Entity.QNAME, pathArgs.get(0).getNodeType());
        assertEquals("Second PathArgument node type", Entity.QNAME, pathArgs.get(1).getNodeType());
        Entry<QName, Object> key = ((NodeIdentifierWithPredicates) pathArgs.get(1)).getKeyValues().entrySet().iterator().next();
        assertEquals("Key node type", QName.create(Entity.QNAME, "name"), key.getKey());
