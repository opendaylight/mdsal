...
class ShardedDOMDataTreeProducer implements DOMDataTreeProducer {
    ...
    private void submitTransaction(final ShardedDOMDataTreeWriteTransaction current) {
        ...
        // FIXME: do not use addCallback, but rather chain the call via lambas
        Futures.addCallback(current.doSubmit(), new FutureCallback<Void>() {
            @Override
            public void onSuccess(final Void result) {
                transactionSuccessful(current, result);
            }

            @Override
            public void onFailure(final Throwable throwable) {
                transactionFailed(current, throwable);
            }
        });
    }
