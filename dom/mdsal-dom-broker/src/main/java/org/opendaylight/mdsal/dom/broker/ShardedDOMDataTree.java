...
public final class ShardedDOMDataTree implements DOMDataTreeService, DOMDataTreeShardingService {
    ...
    private static ListenerRegistration<?> createRegisteredListener(final DOMDataTreeListener userListener,
        ...
        for (Entry<DOMDataTreeShardRegistration<?>, Collection<DOMDataTreeIdentifier>> e : needed.entrySet()) {
            regs.add(ensureListenableShard(e.getKey().getInstance()).registerListener(aggregator.createListener(null),
    ...
    private static ListenableDOMDataTreeShard ensureListenableShard(final DOMDataTreeShard shard) {
        if (shard instanceof ListenableDOMDataTreeShard) {
            return (ListenableDOMDataTreeShard) shard;
        }

        LOG.debug("Using compatibility adaptor for {}", shard);
        return new CompatListenableDOMDataTreeShard(shard);
    }
