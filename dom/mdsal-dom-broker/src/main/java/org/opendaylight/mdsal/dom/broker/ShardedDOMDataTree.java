...
import static org.opendaylight.mdsal.dom.spi.shard.CompatListenableDOMDataTreeShard.createIfNeeded;
    ...
    private static ListenerRegistration<?> createRegisteredListener(final DOMDataTreeListener userListener,
        ...
        if (needed.size() == 1) {
            ...
            final Entry<DOMDataTreeShardRegistration<?>, Collection<DOMDataTreeIdentifier>> e = needed.entrySet()
                    .iterator().next();
            return createIfNeeded(e.getKey().getInstance())
                    .registerListener(userListener, e.getValue(), allowRxMerges);
        }

        // Create an aggregator and start it.
        final int size = needed.size();
        ...
        for (Entry<DOMDataTreeShardRegistration<?>, Collection<DOMDataTreeIdentifier>> e : needed.entrySet()) {
            regs.add(createIfNeeded(e.getKey().getInstance()).registerListener(aggregator.createListener(null),
                e.getValue(), allowRxMerges));
        }

        return aggregator.start(userListener, regs);
    }
