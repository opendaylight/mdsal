...
final class ProducerLayout {
    ...
    private final Set<YangInstanceIdentifier> childRoots;
    ...
    private ProducerLayout(final Map<DOMDataTreeIdentifier, DOMDataTreeShard> shardMap,
        ...
        this.childRoots = Preconditions.checkNotNull(childRoots);
    }

    static ProducerLayout create(final Map<DOMDataTreeIdentifier, DOMDataTreeShard> shardMap) {
        return new ProducerLayout(shardMap, mapIdsToProducer(shardMap), ImmutableMap.of(), ImmutableSet.of());
    }
    ...
    ProducerLayout addChild(final DOMDataTreeProducer producer, final Collection<DOMDataTreeIdentifier> subtrees) {
        ...
        return new ProducerLayout(idToShard, idToProducer, newChildren, ImmutableSet.copyOf(
            Collections2.transform(newChildren.keySet(), DOMDataTreeIdentifier::getRootIdentifier)));
    }

    ProducerLayout reshard(final Map<DOMDataTreeIdentifier, DOMDataTreeShard> shardMap) {
        return new ProducerLayout(shardMap, mapIdsToProducer(shardMap), children, childRoots);
    }
    ...
    void checkAvailable(final YangInstanceIdentifier path) {
        for (YangInstanceIdentifier childPath : childRoots) {
            Preconditions.checkArgument(!childPath.contains(path),
