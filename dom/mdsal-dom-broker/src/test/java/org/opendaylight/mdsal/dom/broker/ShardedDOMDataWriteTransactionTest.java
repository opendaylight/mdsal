...
public class ShardedDOMDataWriteTransactionTest {
    private static final Map<YangInstanceIdentifier, List<NormalizedNode<?, ?>>> testMap = new HashMap<>();
    ...
    public void basicTests() throws Exception {
        ...
        shardedDOMDataWriteTransaction.merge(
                LogicalDatastoreType.OPERATIONAL, yangInstanceIdentifier, TestUtils.TEST_CONTAINER);
        assertTrue(testMap.get(yangInstanceIdentifier).contains(TestUtils.TEST_CONTAINER));
    ...
    private final class TestDOMStoreWriteTransaction implements DOMStoreWriteTransaction{
        ...
        public void write(YangInstanceIdentifier path, NormalizedNode<?, ?> data) {
            List<NormalizedNode<?, ?>> dataList = testMap.get(path);
            if(dataList == null) dataList = new ArrayList<>();
            dataList.add(data);
            testMap.put(path, dataList);
        }
