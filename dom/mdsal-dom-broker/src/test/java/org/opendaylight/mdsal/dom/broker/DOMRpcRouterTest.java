...
public class DOMRpcRouterTest extends TestUtils {
    ...
    public void registerRpcImplementation() {
        try (DOMRpcRouter rpcRouter = new DOMRpcRouter()) {
            ...
            assertFalse(routingTable.getOperations().containsKey(Absolute.of(TestModel.TEST_QNAME)));

            rpcRouter.getRpcProviderService().registerRpcImplementation(getTestRpcImplementation(),
                DOMRpcIdentifier.create(Absolute.of(TestModel.TEST_QNAME), null));
            routingTable = rpcRouter.routingTable();
            assertTrue(routingTable.getOperations().containsKey(Absolute.of(TestModel.TEST_QNAME)));

            rpcRouter.getRpcProviderService().registerRpcImplementation(getTestRpcImplementation(),
                DOMRpcIdentifier.create(Absolute.of(TestModel.TEST2_QNAME), null));
            routingTable = rpcRouter.routingTable();
            assertTrue(routingTable.getOperations().containsKey(Absolute.of(TestModel.TEST2_QNAME)));
        }
    }
    ...
    public void invokeRpc() {
        try (DOMRpcRouter rpcRouter = new DOMRpcRouter()) {
            assertNotNull(rpcRouter.getRpcService().invokeRpc(Absolute.of(TestModel.TEST_QNAME), null));
        }
    }
