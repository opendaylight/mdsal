...
class InmemoryDOMDataTreeShardWriteTransaction implements DOMDataTreeShardWriteTransaction {
    ...
    public ListenableFuture<Boolean> validate() {
        ...
        final ListenableFuture<Boolean> submit = executor.submit(new LoggingTask<>(new ShardCanCommitCoordinationTask(modification.getPrefix(), cohorts)));
        return submit;
    }
    ...
    private static class LoggingTask<T> implements Callable<T> {


        private final Callable<T> callable;

        LoggingTask(final Callable<T> callable) {
        ...
        public T call() throws Exception {
            try {
                ...
                LOG.warn("Uncaught exception during excecution {}", e);
                throw e;
            }
        }
    }
