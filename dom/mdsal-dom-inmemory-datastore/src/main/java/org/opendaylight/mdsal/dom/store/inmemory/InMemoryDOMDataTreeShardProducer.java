...
//      Preconditions.checkState(currentTx == null || currentTx.isFinished(), "Previous transaction not finished yet.");
        ...
        do {
            ...
            ret = parentShard.createTransaction(transactionId, this, prefixes);
        } while (!recordTransaction(entry.getKey(), ret));

        return ret;
    }

    synchronized void transactionReady(final InmemoryDOMDataTreeShardWriteTransaction tx, final DataTreeModification modification) {
        final State localState = state;
        LOG.warn("Transaction was readied {}, current state {}", tx.getIdentifier(), localState);
    ...
    synchronized void onTransactionCommited(final InmemoryDOMDataTreeShardWriteTransaction transaction) {
        ...
        final State localState = state;
        LOG.warn("Transaction {} commit done, current state {}", transaction.getIdentifier(), localState);
        ...
        if (!STATE_UPDATER.compareAndSet(this, localState, idleState)) {
            LOG.debug("Producer {} has already transitioned from {} to {}, not making it idle", this, localState, state);
        }
    }
    ...
    private boolean recordTransaction(final State expected, final InmemoryDOMDataTreeShardWriteTransaction transaction) {
