    ...
    protected synchronized void startConsumption() {
        ...
        scheduledFuture = scheduler.scheduleAtFixedRate(() -> {
            ...
            for (ConsumerRecord<String, byte[]> record : records) {
                try {
                    final DataTreeCandidate dataTreeCandidate = DTCSerializer.deserializeDTC(record.value());
                    applyBackup(dataTreeCandidate);
                } catch (DTCSerializationException e) {
                    LOG.error("Couldn't deserialize DataTreeCandidate from received message.", e);
                }
            }
