...
final class DefaultMatchBuilderValueComparable<R extends DataObject, O extends DataObject, T extends DataObject,
        V extends Comparable<V>>
        extends AbstractMatchBuilderValue<R, O, T, V> implements MatchBuilderValueComparable<T, V> {
    DefaultMatchBuilderValueComparable(final AdaptingQueryBuilder builder, final BoundMethod method) {
        super(builder, method);
    }

    @Override
    public ValueMatch<T> lessThan(final V value) {
        return withPredicate(new LessThan<>(value));
    }

    @Override
    public ValueMatch<T> lessThanOrEqual(final V value) {
        return withPredicate(new LessThanOrEqual<>(value));
    }

    @Override
    public ValueMatch<T> greaterThan(final V value) {
        return withPredicate(new GreaterThan<>(value));
    }

    @Override
    public ValueMatch<T> greaterThanOrEqual(final V value) {
        return withPredicate(new GreaterThanOrEqual<>(value));
    }
}
