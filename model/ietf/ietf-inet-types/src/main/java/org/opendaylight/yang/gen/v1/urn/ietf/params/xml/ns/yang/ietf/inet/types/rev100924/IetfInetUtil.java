...
public final class IetfInetUtil {
    ...
    @Nonnull public static Ipv6Prefix ipv6PrefixFor(@Nonnull final byte[] bytes) {
        return IPV6_PREFIX_FACTORY.newInstance(prefixStringV6(bytes));
    }
    ...
    @Nonnull public static Ipv6Prefix ipv6PrefixFor(@Nonnull final byte[] address, final int mask) {
        return IPV6_PREFIX_FACTORY.newInstance(prefixStringV6(address, mask));
    }
    ...
    private static String prefixStringV6(final byte[] bytes) {
        return addressStringV6(bytes) + "/128";
    }

    private static String prefixStringV6(final byte[] bytes, final int mask) {
        Preconditions.checkArgument(mask >= 0 && mask <= 128, "Invalid mask %s", mask);
        return addressStringV6(bytes) + '/' + mask;
    }
}
